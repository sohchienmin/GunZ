		...
		if (strlen(szMsg) >= 2)
		{
			if (((szMsg[1] > 0) && (isspace(szMsg[1]))) == false)
			{
				...
				if ((ZGetMyInfo()->GetUGradeID() == MMUG_ADMIN) || 
					...
					(ZGetMyInfo()->GetUGradeID() == MMUG_HEADSTAFF))
				{
...
//					return true;			// 채팅창에 표시 안되게 한다.
	...
	else if ((szMsg[0] == '$') && strlen(&szMsg[1]) > 0) {
		ZPOSTCMD1(MC_ADMIN_CHAT, MCommandParameterString(&szMsg[1]));
		return true;
	}
	...
	case GUNZ_GAME:
		{
			...
			if (ZGetGame()->m_pMyCharacter->GetStatus().Ref().Mute == 1)
			{
				ZChatOutput("You're muted!");
				break;
			}
