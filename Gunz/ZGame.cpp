...
#define SNOW_PARTICLE_COUNT_PER_SECOND		400		// ´ë·« ÃÊ´ç 300°³Âë ¿ø·¡ ÇÁ·¹ÀÓ´ç 14°³¿´À½.
...
//	AdjustGlobalTime();
	...
	if(m_pMyCharacter && !m_bReplaying.Ref())
	{
		...
		if(ZGetGame()->GetMatch()->GetMatchType() != MMATCH_GAMETYPE_DUELTOURNAMENT)
			...
			PostDuelTournamentHPAPInfo(); // µà¾óÅä³Ê¸ÕÆ® ¿¡¼­¸¸ Ã³¸®ÇØÁØ´Ù.
	...
	UpdateCombo();	// ÀÌ°ÍÀº ³ªÁß¿¡ MyCharacter·Î µé¾î°¥ ¿¹Á¤
	...
	//////////////////////////////////////////////////////////////////////////
	//	TODO : »óÈ²¿¡ µû¶ó ¾÷µ¥ÀÌÆ® ¾ÈÇØÁàµµ µÉ ¶§¸¦ ±¸º°ÇØ ³¾¼ö ÀÖÀ»±î?
	//////////////////////////////////////////////////////////////////////////
...
void ZGame::CheckMyCharDeadUnchecked()
{
	...
	if ((m_pMyCharacter->IsDie() == true) || (m_pMyCharacter->GetHP() > 0))
		PROTECT_DEBUG_REGISTER(bCheck) return;

	MUID uidAttacker = m_pMyCharacter->GetLastAttacker();//spectate bug
	if (uidAttacker == MUID(0, 0)) return;
...
void ZGame::OnPreDraw()
{
	...
	MComboBox* pWidget = (MComboBox*)ZApplication::GetGameInterface()->GetIDLResource()->FindWidget("MapTexLevel");
	if (pWidget)   
	{
		if (pWidget->GetSelIndex() == 4)
		{
	...
	case MC_GUNZ_DAMAGECOUNTER:
		{
			...
			pCharacter->GetStatus().Ref().nDamage += Damage;
	...
	case MC_PEER_DAMAGE_TAB:
		{
			...
			{
				pCharacter->GetStatus().CheckCrc();
				pCharacter->GetStatus().Ref().nDamage = 0;
				pCharacter->GetStatus().Ref().nDamageTaken = 0;
				pCharacter->GetStatus().MakeCrc();
			}
			/*else
			{
				pCharacter->GetStatus().CheckCrc();
				pCharacter->GetStatus().Ref().nDamage += nDamage;
				pCharacter->GetStatus().Ref().nDamageSoFar += nDamage;
				pCharacter->GetStatus().MakeCrc();
			}*/
		}
		break;
	case MC_PEER_CHAT:
		{
			...
			if (find(ZGetGameClient()->Ignores.begin(), ZGetGameClient()->Ignores.end(), pChar->GetProperty()->GetName()) != ZGetGameClient()->Ignores.end()) break;
...
void ZGame::OnPeerHPInfo(MCommand *pCommand)
{
	...
	// ¿ÉÀúºê ÇÏ°í ÀÖÀ»¶§´Â º¸¿©ÁÖ±â À§ÇØ hp Á¤º¸¸¦ °»½ÅÇÑ´Ù.
	if(ZGetGameInterface()->GetCombatInterface()->GetObserverMode()) {
...
void ZGame::OnPeerDuelTournamentHPAPInfo(MCommand *pCommand)
{
	...
	// ¿ø·¡ peerÀÇ HP/AP Á¤º¸´Â ¿ÀÁ÷ ¿ÉÀú¹ö¿¡°Ô º¸¿©ÁÖ±â À§ÇØ¼­¸¸ Àü´ÞµÈ´Ù.
	// ±âÈ¹»ó µà¾óÅä³Ê¸ÕÆ®ÀÏ ¶§¿¡´Â ½ÇÁ¦ ÇÃ·¹Áß¿¡µµ ´ëÀü »ó´ëÀÇ HP,APÀÇ UI¸¦ ±×·ÁÁÖ±â·Î µÇ¾î ÀÖ´Ù.
	// ¿ÉÁ®¹ö°¡ ¾Æ´Ò¶§(Á÷Á¢ ÇÃ·¡ÀÌ¸¦ ÇÒ¶§)¿¡ peerÀÇ Ä³¸¯ÅÍ HP, AP¸¦ °»½ÅÇØÁÖ¸é 
	// '³» Ä³¸¯ÅÍÀÇ Á×À½Àº ³»°¡ Á÷Á¢ ÆÇ´ÜÇÑ´Ù'´Â ±âÁ¸ p2pÁ¤Ã¥»ó ¹®Á¦°¡ ¹ß»ýÇÒ ¼ö ÀÖ¾î Ä³¸¯ÅÍ¿¡ Á÷Á¢ HP/AP¸¦ setÇÏÁö ¾Ê°í
	// UI Ãâ·Â¿ëÀ¸·Î µû·Î HP/AP°ªÀ» º¸°üÇÑ´Ù.
	if(ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DUELTOURNAMENT) {
...
*//*
	...
	*/
	...
	if (ZGetConfiguration()->GetExtra()->bShotgunSparlke)
	{
...
//			//_ASSERT(FALSE);	// ¹®Á¦°¡ÀÖ´Ù, Ä¡ÆÃ ?
			if(!ZGetGame()->IsReplay())	// ¸®ÇÃ·¹ÀÌ¶ó¸é ÃÑ¾ËÀÇ À¯¹«¿¡ »ó°ü¾øÀÌ ¹ß»çÃ³¸®¸¦ ÇØÁØ´Ù.
				return;	// SHOT
		}
	}
...
//			sprintf(szMsg, "%s´ÔÀÌ ½º½º·Î ÆÐ¹èÇÏ¿´½À´Ï´Ù.", szAttacker);
	...
	else if (pVictim == m_pMyCharacter)
	{
		...
		if (gametype == MMATCH_GAMETYPE_DUEL)
		{
			...
			ZChatOutput(MCOLOR(0xFFCF2020), output);
...
/*
	...
	//°Å¸® °è»êÇÏ°í Áß°£¿¡ º®ÇÏ°í Ãæµ¹ÇÏ´Â Ä³¸¯ÀÎ°¡ °è»êÇÏ°í..
	...
	if( strcmp(g_pGame->m_pMyCharacter->GetProperty()->szName,"¾ËÁö5")!=0 )
		return;
	...
	// ÁÖº¯ÀÇ Ä³¸¯ÅÍ Áß °¡±î¿ì¸é¼­ ¸ÊÀÌ Áß°£¿¡ ÀÖÁö ¾ÊÀº °æ¿ì..³ªÁß¿¡ ÄÚµå ºÎÈ°½Ã..Ã³¸®..
	...
	case MMATCH_ROUNDSTATE_PLAY:
		{
			...
			if (ZGetConfiguration()->GetExtra()->bAutoRecord)
				StartRecording();
	...
	case MMATCH_ROUNDSTATE_FINISH:
		{
			...
			if (ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_DUEL && ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_DUELTOURNAMENT && ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_QUEST && ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_SURVIVAL)
			{
				...
				for (ZCharacterManager::iterator itor = m_CharacterManager.begin(); itor != m_CharacterManager.end(); ++itor)
				{
					...
					float damageDone = pCharacter->GetDamage();
					if (pCharacter->GetTeamID() == ZGetGame()->m_pMyCharacter->GetTeamID()) {
						if (damageDone > TopDamage) {
							TopDamage = damageDone;
							TopPlayer = pCharacter->GetName();
						}
					}
				}
				
				if (ZGetConfiguration()->GetExtra()->bDamageLog)
				{
					char FinishStr[512];
					sprintf(FinishStr, "Best player is %s who has dealt %d damage.", TopPlayer.c_str(), (int)TopDamage);
					ZChatOutput(MCOLOR(124, 252, 0), FinishStr);
					sprintf(FinishStr, "You have dealt %d damage and received %d damage.", m_pMyCharacter->GetStatus().Ref().nDamage, m_pMyCharacter->GetStatus().Ref().nDamageTaken);
					ZChatOutput(MCOLOR(124, 252, 0), FinishStr);
				...
				for(int j=0;j<2;j++)
				{
						...
						itor != ZGetCharacterManager()->end(); ++itor)
					{
						...
						if(pCharacter->IsDie())
						{
							...
							{
								if(!pKiller || pKiller->GetTeamID()==nTeam)			// °°ÀºÆíÇÑÅ× Á×¾úÀ¸¸é ²Î
								{
									bAllKill=false;
									break;
								}
								...
								if(pAllKillPlayer!=pKiller)	// ¿©·¯¸íÀÌ ³ª´² Á×¿´À¸¸é ²Î
								{
									bAllKill=false;
									break;
								}
						...
						{
							bAllKill=false;											// »ì¾ÆÀÖ´Â ³ÑÀÌ ÀÖ¾îµµ ²Î
							break;
						}
					}
...
void ZGame::PostSpMotion(ZC_SPMOTION_TYPE mtype)
{
		...
		(m_pMyCharacter->m_AniState_Lower.Ref() == ZC_STATE_LOWER_IDLE4) ) 
	{
		...
		if( mtype == ZC_SPMOTION_TAUNT ) // taunt ÀÏ °æ¿ì ¸ð¼ÇÀÌ ¾ø¾î¼­...
			if( (type == MWT_MED_KIT) || 
				(type == MWT_REPAIR_KIT) || 
				(type == MWT_FOOD) ||
				(type == MWT_BULLET_KIT)) 
