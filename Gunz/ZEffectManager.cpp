...
//	m_pEBSBulletMark[1] = new ZEffectBillboardSource("SFX/bullet-mark02.tga");
	...
	m_pDashEffectRed			 = m_pEffectMeshMgr->Get("dash_effect_red");
	m_pDashEffectGreen			 = m_pEffectMeshMgr->Get("dash_effect_green");
	m_pDashEffectBlue			 = m_pEffectMeshMgr->Get("dash_effect_blue");
	m_pDashEffectCyan			 = m_pEffectMeshMgr->Get("dash_effect_cyan");
	m_pDashEffectPurple			 = m_pEffectMeshMgr->Get("dash_effect_purple");
	m_pDashEffectYellow			 = m_pEffectMeshMgr->Get("dash_effect_yellow");
...
void ZEffectManager::AddDashEffect(rvector& Target, rvector& TargetNormal, ZObject* pObj, int nDashColor)
{
	...
	ZEffect* pNew = NULL;
	RMesh*	m_pNewDashEffect;
	switch (nDashColor)
	{
		case 1013:
			m_pNewDashEffect = m_pDashEffectRed;
		break;
		case 1014:
			m_pNewDashEffect = m_pDashEffectGreen;
			break;
		case 1015:
			m_pNewDashEffect = m_pDashEffectBlue;
			break;
		case 1016:
			m_pNewDashEffect = m_pDashEffectYellow;
			break;
		case 1017:
			m_pNewDashEffect = m_pDashEffectCyan;
			break;
		case 1018:
			m_pNewDashEffect = m_pDashEffectPurple;
			break;
		default:
			m_pNewDashEffect = m_pDashEffect;
	}

	pNew = new ZEffectDash(m_pNewDashEffect, Target, TargetNormal, pObj->GetUID());
	((ZEffectSlash*)pNew)->SetAlignType(1);
	Add(pNew);
}
#pragma optimize ( "", on)
