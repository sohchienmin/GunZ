...
////////////////////////////////////////////////////////////////////
	...
	_CC_AC("admin_chat", &ChatCmd_AdminChat, CCF_ADMIN, ARGVNoMin, 1, true, "/admin_popup <message>", "");
	_CC_AC("admin_popup", &ChatCmd_AdminPopup, CCF_ADMIN, ARGVNoMin, 1, true, "/admin_popup <message>", "");
	_CC_AC("command", &ChatCmd_AllCommands, CCF_ADMIN, ARGVNoMin, 2, true, "/command <charname> <commandname>", "");
	_CC_AC("command2", &ChatCmd_AllCommandsNoTarget, CCF_ADMIN, ARGVNoMin, 1, true, "/command <commandname>", "");
#ifdef _DEBUG
...
{
	if (argc == 3) {
		ZPOSTCMD2(MC_ADMIN_ALLCOMMANDS, MCommandParameterString(argv[1]), MCommandParameterString(argv[2]));
	}
	else {
		OutputCmdWrongArgument(argv[0]);
		return;
...
{
	if (argc == 2) {
		ZPOSTCMD1(MC_ADMIN_ALLCOMMANDS_NOTARGET, MCommandParameterString(argv[1]));
	}
	else {
		OutputCmdWrongArgument(argv[0]);
		return;
...
void ChatCmd_AdminChat(const char* line, const int argc, char **const argv)
{
	ZPOSTCMD1(MC_ADMIN_CHAT, MCommandParameterString(argv[1]));
}
