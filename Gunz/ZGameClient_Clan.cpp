...
void ZGameClient::RequestCreateClan(char* szClanName, char* szClanPassword)
{
	if (ZGetMyInfo()->IsClanJoined())
	{
		ZChatOutput(ZMsg(MSG_CLAN_JOINED_ALREADY), ZChat::CMT_SYSTEM);
		return;
	}
	if (ZApplication::GetGameInterface()->GetState() != GUNZ_LOBBY)
	{
	...
	ZPostRequestCreateClan(szClanName, szClanPassword);
}
...
{
	if (find(Ignores.begin(), Ignores.end(), szSenderName) != Ignores.end()) return;
	char szText[512];
...
void ZGameClient::OnClanStandbyClanList(int nPrevStageCount, int nNextStageCount, void* pBlob)
{
	...
	for(int i=0; i<nCount; i++) 
	{
		...
		//_ASSERT(i<4);
		pListBox->SetInfo(i,pNode->nCLID,pNode->szClanName, pNode->bDodge, pNode->nPlayers);
