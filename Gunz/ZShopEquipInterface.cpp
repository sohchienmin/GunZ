...
void ZShopEquipInterface::DrawCharInfoText(char* szShopOrEquip, int nReqLevel, int nNewWT, int nNewMaxWT, int nNewHP, int nNewAP, int nReqBounty)
{
	...
	sprintf(sz1, "^9Cash : ");
	szColor = (nReqBounty > ZGetMyInfo()->GetBPPlus()) ? szGray : szGray;
	sprintf(sz2, "%s%d", szColor, ZGetMyInfo()->GetBPPlus());
	strcat(sz1, sz2);
	pTextArea[2]->AddText(sz1);

	sprintf(sz1, "^9EventCoins : ");
	szColor = (nReqBounty > ZGetMyInfo()->GetEventCoins()) ? szGray : szGray;
	sprintf(sz2, "%s%d", szColor, ZGetMyInfo()->GetEventCoins());
	strcat(sz1, sz2);
