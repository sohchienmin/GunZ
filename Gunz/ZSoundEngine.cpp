...
#define MUSIC_FOLDER		"/Music"
	memset(m_szCustomMusicNames, 0, sizeof(m_szCustomMusicNames));
	if (GetMyDocumentsPath(szFold))
	{
		strcat(szFold, GUNZ_FOLDER);
		if (GetFileAttributes(szFold) == INVALID_FILE_ATTRIBUTES)
			CreatePath(szFold);
		strcat(szFold, MUSIC_FOLDER);
		if (GetFileAttributes(szFold) == INVALID_FILE_ATTRIBUTES)
			CreatePath(szFold);
		strcpy(m_szMusicPath, szFold);
	}
	strcpy(szTempFolder, m_szMusicPath);
	strcat(szTempFolder, "\\*");
	...
	do
	{
		...
		{
			strcpy(m_szCustomMusicNames[nFileCounter], w32Data.cFileName);
			++nFileCounter;
		}
		++nTempSkipToTwo;
	} while (FindNextFile(hFile, &w32Data) != false);
	m_nNumOfMusics = nFileCounter;
	m_bMusicEnded = true;
}
FILE* pFile;
bool ZSoundEngine::OpenCustomMusic(const char* szFileNameDir, const char* szFileName)
{
	if (!m_bSoundEnable)
		return false;
	...
	m_pMusicBuffer = new char[nLen + 1];

	fread(m_pMusicBuffer, nLen, 1, pFile);
	m_pMusicBuffer[nLen] = 0;
	m_bMusicEnded = false;
	return ZGetSoundFMod()->OpenStream(m_pMusicBuffer, nLen);
}
bool ZSoundEngine::PlayAudio(int nPlayNumb)
{
	char szTemp[MAX_PATH];
	sprintf(szTemp, "%s\\%s", m_szMusicPath, m_szCustomMusicNames[nPlayNumb]);
	m_nLastSongIndex = nPlayNumb;

	if (OpenCustomMusic(szTemp, m_szCustomMusicNames[nPlayNumb]))
		return true;
...
void ZSoundEngine::PlaySEFire(MMatchItemDesc *pDesc, float x, float y, float z, bool bPlayer)
{
	...
	if (pDesc->m_nType.Ref() == MMIT_RANGE || pDesc->m_nType.Ref() == MMIT_CUSTOM)
	{
		...
		{
			char szBuffer[64];
			if(ZGetConfiguration()->GetAudio()->b3DSounde)
