...
class ZIgnorePlayerListItem : public ZPlayerListItem{
protected:
...
public:
	ZIgnorePlayerListItem(const MUID& puid, MBitmap* pBitmap, const char* szName, const char* szClanName, const char* szLocation,
...
//	MBitmap* GetBitmapIn() { return m_pBitmapIn; }
	...
	void AddPlayer(MUID& puid, ePlayerState state, int  nLevel, char* szName, char* szClanName, unsigned int nClanID, MMatchUserGradeID nGrade, int duelTournamentGrade, int nR, int nG, int nB);
	...
	void AddIgnore(ePlayerState state, char* szName, char* szLocation);
