...
#define ZPostDash(/*rvector&*/ vPos, /*rvector&*/ vDir, /*unsigned char*/ sel_type, nDash) { \
	ZPACKEDDASHINFO pdi;		\
...
inline void ZPostChannelRequestJoin(const MUID& uidChar, const MUID& uidChannel, const char* szChannelPassword)
{
	ZPOSTCMD3(MC_MATCH_CHANNEL_REQUEST_JOIN, MCommandParameterUID(uidChar), MCommandParameterUID(uidChannel), MCommandParameterString(szChannelPassword));
}
...
#define ZPostChannelChat(/*const MUID&*/ uidChar, /*const MUID&*/ uidChannel, /*char**/ szChat) { \
	ZPOSTCMD6(CLOAK_CMD_ID(MC_MATCH_CHANNEL_REQUEST_CHAT, 87252), MCommandParameterUID(uidChar), MCommandParameterUID(uidChannel), MCmdParamStr(szChat), MCommandParameterInt(ZGetMyInfo()->GetRed()), MCommandParameterInt(ZGetMyInfo()->GetGreen()), MCommandParameterInt(ZGetMyInfo()->GetBlue()));\
}
...
inline void ZPostIgnoreAdd(const char* pszName)
{
	ZPOSTCMD1(MC_MATCH_IGNORE_ADD, MCmdParamStr(const_cast<char*>(pszName)));
}

inline void ZPostIgnoreRemove(const char* pszName)
{
	ZPOSTCMD1(MC_MATCH_IGNORE_REMOVE, MCmdParamStr(const_cast<char*>(pszName)));
}

inline void ZPostIgnoreList()
{
	ZPOSTCMD0(MC_MATCH_IGNORE_LIST);
}
...
inline void ZPostCreateMyChar(const MUID& uidChar, const int nCharIndex, char* szCharName,
	const int nSex, const int nHair, const int nFace, const int nCostume, int nRed, int nGreen, int nBlue)
{
	ZPOSTCMD10(MC_MATCH_REQUEST_CREATE_CHAR, MCommandParameterUID(uidChar), MCommandParameterUInt(nCharIndex),
		MCommandParameterString(szCharName), MCommandParameterUInt(nSex), MCommandParameterUInt(nHair),
		MCommandParameterUInt(nFace), MCommandParameterUInt(nCostume), MCommandParameterInt(nRed), MCommandParameterInt(nGreen), MCommandParameterInt(nBlue));
...
inline void ZPostRequestBuyCashItem(const MUID& uidChar, unsigned int nItemID, unsigned int nItemCnt = 1, bool IsUnlimited = true)
{
	ZPOSTCMD4(MC_MATCH_REQUEST_BUY_CASH_ITEM, MCommandParameterUID(uidChar), MCommandParameterUInt(nItemID), MCommandParameterUInt(nItemCnt), MCommandParameterBool(IsUnlimited));
}
...
inline void ZPostRequestCreateClan(char* szClanName, char* szClanPassword)
{
	ZPOSTCMD2(MC_MATCH_CLAN_REQUEST_CREATE_CLAN, MCmdParamStr(szClanName), MCmdParamStr(szClanPassword));
}
