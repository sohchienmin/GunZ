...
void ZOptionInterface::InitInterfaceOption(void)
{
	...
	*/
	...
	pWidget = (MSlider*)pResource->FindWidget("SwordTrailSize");
	if (pWidget)
	{
		pWidget->SetMinMax(Z_SWORD_TRAILSIZE, Z_SWORD_TRAILSIZE_MAX);
		pWidget->SetValue(ZGetConfiguration()->GetSwordTrailSizeInInt());
	}

	pWidget = (MSlider*)pResource->FindWidget("CharacterZoomSlider");
	if (pWidget)
	{
		pWidget->SetMinMax(Z_CHARACTER_ZOOM, Z_CHARACTER_ZOOM_MAX);
		pWidget->SetValue(ZGetConfiguration()->GetCharacterZoomInInt());
	}
	...
	//	Button
	{
		...
		pWidget = (MButton*)pResource->FindWidget("Effect3De");
		if (pWidget)
		{
			pWidget->SetCheck(ZGetConfiguration()->GetAudio()->b3DSounde);
		}
	...
	{	
		...
		{
			char szBuf[64];
			sprintf(szBuf, "%i", ZGetConfiguration()->m_Etc.nFrameLimit_perSecond);
			pEdit->SetText(szBuf);
			pEdit->SetMaxLength(3);
			RSetFrameLimitPerSeceond(ZGetConfiguration()->m_Etc.nFrameLimit_perSecond);
		}
	}
	...
	//Extra
	{
		...
		ZCONFIG_EXTRA* pExtra = ZGetConfiguration()->GetExtra();


		MButton* pButton = (MButton*)pResource->FindWidget("ItemsScroll");
		if (pButton)
		{
			pButton->SetCheck(Z_EXTRA_ITEMSSCROLL);
		}

		pButton = (MButton*)pResource->FindWidget("CharacterVisibility");
		if (pButton)
		{
			pButton->SetCheck(Z_EXTRA_CHARACTERVISIBILITY);
		}

		pButton = (MButton*)pResource->FindWidget("AutoRecord");
		if (pButton)
		{
			pButton->SetCheck(Z_EXTRA_AUTORECORD);
		}

		pButton = (MButton*)pResource->FindWidget("DamageLog");
		if (pButton)
		{
			pButton->SetCheck(Z_EXTRA_DAMAGELOG);
		}

		pButton = (MButton*)pResource->FindWidget("Bullet");
		if (pButton)
		{
			pButton->SetCheck(Z_EXTRA_BULLETSHOLES);
		}


		pButton = (MButton*)pResource->FindWidget("ShotgunSparlke");
		if (pButton)
		{
			pButton->SetCheck(Z_EXTRA_SHOTGUNSPARLKE);
		}

		pButton = (MButton*)pResource->FindWidget("HPAPFPSBar");
		if (pButton)
		{
			pButton->SetCheck(Z_EXTRA_INFOBAR);
		}
...
bool ZOptionInterface::SaveInterfaceOption(void)
{
	...
	{ // 슬라이더
		...
		pWidget = (MSlider*)pResource->FindWidget("SwordTrailSize");
		Z_EXTRA_SWORDTRAIL = pWidget->GetValue();

		pWidget = (MSlider*)pResource->FindWidget("CharacterZoomSlider");
		Z_EXTRA_CHARACTERZOOM = pWidget->GetValue();
...
//		ZGetInput()->UnregisterActionKey(i);
	...
	*/
	...
	}
	{
		...
		if (pWidget)
		{
			if (Z_AUDIO_3D_SOUNDe != pWidget->GetCheck()) {
				Z_AUDIO_3D_SOUNDe = pWidget->GetCheck();
				if (Z_AUDIO_3D_SOUNDe)
					ZGetConfiguration()->GetAudio()->b3DSounde = true;
				else
					ZGetConfiguration()->GetAudio()->b3DSounde = false;
			}
		}
		pWidget = (MButton*)pResource->FindWidget("8BitSound");
		if(pWidget)
...

	{
		...
		if (pButton)
		{
			if (Z_EXTRA_ITEMSSCROLL != pButton->GetCheck()) {
				Z_EXTRA_ITEMSSCROLL = pButton->GetCheck();
				if (Z_EXTRA_ITEMSSCROLL)
					ZGetConfiguration()->GetExtra()->bSpecialItemsScroll = true;
				else
					ZGetConfiguration()->GetExtra()->bSpecialItemsScroll = false;
			}
		}
		...
		if (pButton)
		{
			if (Z_EXTRA_CHARACTERVISIBILITY != pButton->GetCheck()) {
				Z_EXTRA_CHARACTERVISIBILITY = pButton->GetCheck();
				if (Z_EXTRA_CHARACTERVISIBILITY)
					ZGetConfiguration()->GetExtra()->bCharacterVisibility = true;
				else
					ZGetConfiguration()->GetExtra()->bCharacterVisibility = false;
			}
		}
		...
		if (pButton)
		{
			if (Z_EXTRA_AUTORECORD != pButton->GetCheck()) {
				Z_EXTRA_AUTORECORD = pButton->GetCheck();
				if (Z_EXTRA_AUTORECORD)
					ZGetConfiguration()->GetExtra()->bAutoRecord = true;
				else
					ZGetConfiguration()->GetExtra()->bAutoRecord = false;
			}
		}
		...
		if (pButton)
		{
			if (Z_EXTRA_DAMAGELOG != pButton->GetCheck()) {
				Z_EXTRA_DAMAGELOG = pButton->GetCheck();
				if (Z_EXTRA_DAMAGELOG)
					ZGetConfiguration()->GetExtra()->bDamageLog = true;
				else
					ZGetConfiguration()->GetExtra()->bDamageLog = false;
			}
		}
		...
		if (pButton)
		{
			if (Z_EXTRA_BULLETSHOLES != pButton->GetCheck()) {
				Z_EXTRA_BULLETSHOLES = pButton->GetCheck();
				if (Z_EXTRA_BULLETSHOLES)
					ZGetConfiguration()->GetExtra()->bBulletsholes = true;
				else
					ZGetConfiguration()->GetExtra()->bBulletsholes = false;
			}
		}
		...
		if (pButton)
		{
			if (Z_EXTRA_SHOTGUNSPARLKE != pButton->GetCheck()) {
				Z_EXTRA_SHOTGUNSPARLKE = pButton->GetCheck();
				if (Z_EXTRA_SHOTGUNSPARLKE)
					ZGetConfiguration()->GetExtra()->bShotgunSparlke = true;
				else
					ZGetConfiguration()->GetExtra()->bShotgunSparlke = false;
			}
		}
		pButton = (MButton*)pResource->FindWidget("HPAPFPSBar");
		if (pButton)
		{
			if (Z_EXTRA_INFOBAR != pButton->GetCheck()) {
				Z_EXTRA_INFOBAR = pButton->GetCheck();
				if (Z_EXTRA_INFOBAR)
					ZGetConfiguration()->GetExtra()->bInfoBar = true;
				else
					ZGetConfiguration()->GetExtra()->bInfoBar = false;
			}
		}
...
BEGIN_IMPLEMENT_LISTENER(ZGetOptionFrameButtonListener, MBTN_CLK_MSG)
	...
	MComboBox* pMusic = (MComboBox*)pResource->FindWidget("RefreshCustomMusic");
	pMusic->RemoveAll();
	for(int j=0 ; j< ZGetSoundEngine()->m_nNumOfMusics ; j++)
		pMusic->Add((char*)ZGetSoundEngine()->m_szCustomMusicNames[j]);
	if(strlen(pMusic->GetText()) == 0)
		pMusic->SetText("Music List");

	MButton* pButton = (MButton*)pResource->FindWidget("BeforeButton");
	pButton->SetText("<");
	pButton = (MButton*)pResource->FindWidget("PlayButton");
	pButton->SetText("P");
	pButton = (MButton*)pResource->FindWidget("AfterButton");
	pButton->SetText(">");
#ifdef LOCALE_NHNUSA
...
BEGIN_IMPLEMENT_LISTENER(ZGetSaveOptionButtonListener, MBTN_CLK_MSG)
	...
	MFrame* pWidget = (MFrame*)pResource->FindWidget("OptionGroup");
	MBFrameLook* pFrameLook = (MBFrameLook*)pResource->FindFrameLook("DefaultFrameLook");
	pWidget->ChangeCustomLook((MFrameLook*)pFrameLook);
...
//	ZApplication::GetGameInterface()->SaveInterfaceOption();
	ZIDLResource* pResource = ZApplication::GetGameInterface()->GetIDLResource();
	MFrame* pLOLO = (MFrame*)pResource->FindWidget("OptionGroup");
	MBFrameLook* pFrameLook = (MBFrameLook*)pResource->FindFrameLook("DefaultFrameLook");
	pLOLO->ChangeCustomLook((MFrameLook*)pFrameLook);
	// TODO: 이게 필요한가 ? 테스트 요망
	if( pWidget->m_bEventAcceleratorCall ) {
...
static void SetSwordTrailSizeSlider(int i)
{
	ZIDLResource* pResource = ZApplication::GetGameInterface()->GetIDLResource();
	MSlider* pSlider = (MSlider*)pResource->FindWidget("SwordTrailSize");
	if (pSlider)
	{
		pSlider->SetValue(i);
	}
...
static void SetCharacterZoomSlider(int i)
{
	ZIDLResource* pResource = ZApplication::GetGameInterface()->GetIDLResource();
	MSlider* pSlider = (MSlider*)pResource->FindWidget("CharacterZoomSlider");
	if (pSlider)
	{
		pSlider->SetValue(i);
	}
...
BEGIN_IMPLEMENT_LISTENER(ZGetSwordTrailSizeSliderListener, MLIST_VALUE_CHANGED)
	ZIDLResource* pResource = ZApplication::GetGameInterface()->GetIDLResource();
	MSlider* pSlider = (MSlider*)pResource->FindWidget("SwordTrailSize");
	if (pSlider != NULL)
	{
		int i = ZGetConfiguration()->ValidateSwordTrailSizeInInt(pSlider->GetValue());
		SetSwordTrailSizeSlider(i);
		MPicture* pPicture = (MPicture*)ZApplication::GetGameInterface()->GetIDLResource()->FindWidget("SwordTrailSizeBitMap");
		MSlider* pSize = (MSlider*)pResource->FindWidget("SwordTrailSize");
		char szSize[128];
		sprintf(szSize, "swordtrail_%d.png", (int)pSize->GetValue());
		pPicture->SetBitmap(MBitmapManager::Get(szSize));
	}
END_IMPLEMENT_LISTENER()
...
BEGIN_IMPLEMENT_LISTENER(ZGetCharacterZoomSliderListener, MLIST_VALUE_CHANGED)
	ZIDLResource* pResource = ZApplication::GetGameInterface()->GetIDLResource();
	MSlider* pSlider = (MSlider*)pResource->FindWidget("CharacterZoomSlider");
	if (pSlider != NULL)
	{
		int i = ZGetConfiguration()->ValidateCharacterZoomInInt(pSlider->GetValue());
		SetCharacterZoomSlider(i + 290);
		ZGetCamera()->m_fDist = pSlider->GetValue() + 290;
		if (ZGetGame())
		{
			MFrame* pWidget = (MFrame*)pResource->FindWidget("OptionGroup");
			MBFrameLook* pFrameLook = (MBFrameLook*)pResource->FindFrameLook("HiddenFrameLook");
			pWidget->ChangeCustomLook((MFrameLook*)pFrameLook);
		}
	}
END_IMPLEMENT_LISTENER()
