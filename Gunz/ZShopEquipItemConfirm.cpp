...
void ZItemCountDlg::Open(ZITEMCOUNTDLG_MODE mode, const char* szItemName, MBitmap* pIcon, int price, int unlimitedprice, bool bIsCash, bool bIsEvent, int nMin, int nMax, IItemCountDlgDoneHandler* pHandler)
{
	m_mode = mode;
	m_strItemName = szItemName;
	m_pItemIcon = pIcon;
	m_nPrice = price;
	m_nUnlimitedPrice = unlimitedprice;
	m_nMin = nMin;
	m_nMax = nMax;
	...
	MLabel* pLabel = (MLabel*)GetIDLResource()->FindWidget("UnlimitedRentLabel");
	MButton* pButton = (MButton*)GetIDLResource()->FindWidget("UnlimitedRent");
	...
	switch (m_mode) {
		...
		case ZICD_BUYCASH:
		{
			...
			if (pFrame)
			{
				pFrame->SetFont(MFontManager::Get("FONTa10_O2Wht"));
				pFrame->SetText("Purchase donator item");
			}
			if (pLabel1)
			{
				pLabel1->SetFont(MFontManager::Get("FONTa10_O2Wht"));
				pLabel1->SetText("Please enter amount of days to rent");
			}
			if (pLabel2)
			{
				pLabel2->SetFont(MFontManager::Get("FONTa10_O2Wht"));
				pLabel2->SetText("Rent Day(s)");
			}
			if (pBtn)
			{
				pBtn->SetText("Rent");
...
void ZItemCountDlg::UpdateDlg()
{
	...
	if (pLabel)
	{
		...
		else if (m_mode == ZICD_BUYCASH)
		{
			if (pButton->GetCheck())
				sprintf(szText, "%s x Unlimited Days", m_strItemName.c_str());
			else
				sprintf(szText, "%s (%d Cash) x %d Days", m_strItemName.c_str(), m_nPrice, m_nCurrCount);
	...
	{	
		...
		if (m_nCurrCount == 0 && !pButton->GetCheck())
			pBtn->Enable(false);
		else
			pBtn->Enable(true);
		pLabel = (MLabel*)GetIDLResource()->FindWidget("TradeCountableItem_Total");
		if (pLabel)
		{
			...
			if (pButton->GetCheck())
			{
				pLabel = (MLabel*)ZGetGameInterface()->GetIDLResource()->FindWidget("TradeCountableItem_Total");
				char buff[124];
				sprintf(buff, "= %d Cash", m_nUnlimitedPrice);

				pLabel->SetText(buff);
			}
			else
			{
				sprintf(szText, "= %d Cash", m_nPrice * m_nCurrCount);
				pLabel->SetText(szText);
			}
		}
	}
