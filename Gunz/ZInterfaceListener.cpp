...
BEGIN_IMPLEMENT_LISTENER(ZGetShopCloseButtonListener, MBTN_CLK_MSG)
	MComboBox* pMainFilter = (MComboBox*)ZApplication::GetGameInterface()->GetIDLResource()->FindWidget("Shop_AllEquipmentFilter");
	pMainFilter->SetSelIndex(0);
	MComboBox* pSubFilter = (MComboBox*)ZApplication::GetGameInterface()->GetIDLResource()->FindWidget("Shop_AllEquipmentItemFilter");
	pSubFilter->SetSelIndex(0);
	pSubFilter->CloseComboBoxList();
	pSubFilter->Enable(false);
	pSubFilter->RemoveAll();
	pSubFilter->SetText("Sub Filter");
	ZGetShop()->m_ListFilter = 0;
	ZGetShop()->Serialize();
...
BEGIN_IMPLEMENT_LISTENER(ZGetEquipmentCallerButtonListener, MBTN_CLK_MSG)
	...
	ZGetMyInfo()->GetItemList()->SerializeQuest();
END_IMPLEMENT_LISTENER()

BEGIN_IMPLEMENT_LISTENER(ZGetEquipmentCloseButtonListener, MBTN_CLK_MSG)
	MComboBox* pMainFilter = (MComboBox*)ZApplication::GetGameInterface()->GetIDLResource()->FindWidget("Equip_AllEquipmentFilter");
	pMainFilter->SetSelIndex(0);
	MComboBox* pSubFilter = (MComboBox*)ZApplication::GetGameInterface()->GetIDLResource()->FindWidget("Equip_AllEquipmenItemtFilter");
	pSubFilter->SetSelIndex(0);
	pSubFilter->CloseComboBoxList();
	pSubFilter->Enable(false);
	pSubFilter->RemoveAll();
	pSubFilter->SetText("Sub Filter");
	ZGetMyInfo()->GetItemList()->m_ListFilter = 0;
	ZGetMyInfo()->GetItemList()->SerializeZItemList();
	ZGetMyInfo()->GetItemList()->SerializeQuest();
...
BEGIN_IMPLEMENT_LISTENER(ZGetUnlimitedRentButtonListener, MBTN_CLK_MSG)
	MButton* pButton = (MButton*)ZGetGameInterface()->GetIDLResource()->FindWidget("UnlimitedRent");
	if (pButton->GetCheck())
	{
		MLabel* pLabel = (MLabel*)ZGetGameInterface()->GetIDLResource()->FindWidget("TradeCountableItem_Calculate");
		pLabel->SetText("Unlimited Days");

		pButton = (MButton*)ZGetGameInterface()->GetIDLResource()->FindWidget("TradeCountableItem_CountUp");
		pButton->Enable(false);

		pButton = (MButton*)ZGetGameInterface()->GetIDLResource()->FindWidget("TradeCountableItem_CountDn");
		pButton->Enable(false);

		MEdit* pEdit = (MEdit*)ZGetGameInterface()->GetIDLResource()->FindWidget("TradeCountableItem_CountNum");
		pEdit->SetText("Unlimited");
		pEdit->Enable(false);
	}
	else
	{
...
END_IMPLEMENT_LISTENER()
BEGIN_IMPLEMENT_LISTENER(ZGetCreateCharacterRedColorListener, MLIST_VALUE_CHANGED)
ZIDLResource* pResource = ZGetGameInterface()->GetIDLResource();
...
pBlueColor = (MSlider*)pResource->FindWidget("BlueColor");
pRedColor->SetMinMax(0, 255);
...
BEGIN_IMPLEMENT_LISTENER(ZGetCreateCharacterGreenColorListener, MLIST_VALUE_CHANGED)
ZIDLResource* pResource = ZGetGameInterface()->GetIDLResource();
MEdit* pEdit = (MEdit*)pResource->FindWidget("CC_CharName");
MSlider* pRedColor, *pGreenColor, *pBlueColor;
pRedColor = (MSlider*)pResource->FindWidget("RedColor");
...
BEGIN_IMPLEMENT_LISTENER(ZGetCreateCharacterBlueColorListener, MLIST_VALUE_CHANGED)
ZIDLResource* pResource = ZGetGameInterface()->GetIDLResource();
MEdit* pEdit = (MEdit*)pResource->FindWidget("CC_CharName");
MSlider* pRedColor, *pGreenColor, *pBlueColor;
pRedColor = (MSlider*)pResource->FindWidget("RedColor");
...
BEGIN_IMPLEMENT_LISTENER(ZGetShowCreateCharacterButtonListener, MBTN_CLK_MSG)
	...
	if (ZGetMyInfo()->GetUGradeID() == MMUG_FREE)
	{
		...
		pGreenColor->SetMinMax(0, 255);
		pBlueColor->SetMinMax(0, 255);
		pRedColor->SetValue(255);
		pGreenColor->SetValue(255);
		pBlueColor->SetValue(255);
		pRedColor->Enable(false);
		pGreenColor->Enable(false);
		pBlueColor->Enable(false);
		MLabel* pColorLabel = (MLabel*)pResource->FindWidget("LabelNameColor");
		pColorLabel->SetText("");
...
BEGIN_IMPLEMENT_LISTENER(ZGetCreateCharacterButtonListener, MBTN_CLK_MSG)
	...
	if(nEmptySlotIndex>=0)
	{
		...
		ZPostCreateMyChar(ZGetGameClient()->GetPlayerUID(), nEmptySlotIndex, (char*)pEdit->GetText(), pSexCB->GetSelIndex(),
			pHairCB->GetSelIndex(), pFaceCB->GetSelIndex(), pCostumeCB->GetSelIndex(), pRedColor->GetValue(), pGreenColor->GetValue(), pBlueColor->GetValue());
...
BEGIN_IMPLEMENT_LISTENER(ZGetPlayerListPrevListener, MBTN_CLK_MSG)
{
	...
	if (pWidget->GetMode() == ZPlayerListBox::PLAYERLISTMODE_CHANNEL_FRIEND || pWidget->GetMode() == ZPlayerListBox::PLAYERLISTMODE_CHANNEL_IGNORE) {
...
BEGIN_IMPLEMENT_LISTENER(ZGetNewsListener, MBTN_CLK_MSG)
	MFrame* pFrame = (MFrame*)ZGetGameInterface()->GetIDLResource()->FindWidget("NewsFrame");
	if (pFrame->IsVisible())
	{
		MListBox* pBox = (MListBox*)ZGetGameInterface()->GetIDLResource()->FindWidget("News_list");
		pBox->RemoveAll();
...
BEGIN_IMPLEMENT_LISTENER(ZGetDefaultCharacterZoomListener, MBTN_CLK_MSG)
	ZIDLResource* pResource = ZApplication::GetGameInterface()->GetIDLResource();
	MSlider* pSlider = (MSlider*)pResource->FindWidget("CharacterZoomSlider");
	if (pSlider != NULL)
	{
		ZGetCamera()->m_fDist = 290.f;
		pSlider->SetValue(0);
	}
END_IMPLEMENT_LISTENER()

BEGIN_IMPLEMENT_LISTENER(ZGetUpdateClanPassword, MBTN_CLK_MSG)

MEdit* pClanPassword = (MEdit*)ZGetGameInterface()->GetIDLResource()->FindWidget("ClanPasswordUpdate");
ZPOSTCMD1(MC_MATCH_UPDATE_PASSWORD, MCmdParamStr(pClanPassword->GetText()));

END_IMPLEMENT_LISTENER()

BEGIN_IMPLEMENT_LISTENER(ZGetLockMyChannel, MBTN_CLK_MSG)

MFrame* pFrame = (MFrame*)ZGetGameInterface()->GetIDLResource()->FindWidget("ClanUpdatePassword");
pFrame->Show(true, true);
MEdit* pClanName = (MEdit*)ZGetGameInterface()->GetIDLResource()->FindWidget("ClanNameUpdate");
pClanName->SetText(ZGetMyInfo()->GetClanName());
pClanName = (MEdit*)ZGetGameInterface()->GetIDLResource()->FindWidget("ClanPasswordUpdate");
pClanName->SetText("");
END_IMPLEMENT_LISTENER()

BEGIN_IMPLEMENT_LISTENER(ZGetSumbitPassword, MBTN_CLK_MSG)
MEdit* pPassEdit = (MEdit*)ZGetGameInterface()->GetIDLResource()->FindWidget("ClanPasswordSubmit");
ZPostChannelRequestJoin(ZGetMyUID(), ZGetGameClient()->GetTempChannelUID(), pPassEdit->GetText());
Sleep(100);
END_IMPLEMENT_LISTENER()


BEGIN_IMPLEMENT_LISTENER(ZGetBeforeButtonListener, MBTN_CLK_MSG)
ZIDLResource* pResource = ZApplication::GetGameInterface()->GetIDLResource();
MComboBox* pMusic = (MComboBox*)pResource->FindWidget("RefreshCustomMusic");
ZGetSoundEngine()->PlayAudio(pMusic->GetSelIndex() - 1);
ZGetSoundEngine()->PlayMusic(true);
pMusic->SetSelIndex(pMusic->GetSelIndex() - 1);
END_IMPLEMENT_LISTENER()
BEGIN_IMPLEMENT_LISTENER(ZGetPlayButtonListener, MBTN_CLK_MSG)
ZIDLResource* pResource = ZApplication::GetGameInterface()->GetIDLResource();
MComboBox* pMusic = (MComboBox*)pResource->FindWidget("RefreshCustomMusic");
ZGetSoundEngine()->PlayAudio(pMusic->GetSelIndex());
ZGetSoundEngine()->PlayMusic(true);
END_IMPLEMENT_LISTENER()

BEGIN_IMPLEMENT_LISTENER(ZGetAfterButtonListener, MBTN_CLK_MSG)
ZIDLResource* pResource = ZApplication::GetGameInterface()->GetIDLResource();
MComboBox* pMusic = (MComboBox*)pResource->FindWidget("RefreshCustomMusic");
ZGetSoundEngine()->PlayAudio(pMusic->GetSelIndex() + 1);
ZGetSoundEngine()->PlayMusic(true);
...
BEGIN_IMPLEMENT_LISTENER(ZGetUnlockMyChannel, MBTN_CLK_MSG)

ZPOSTCMD0(MC_MATCH_REMOVE_PASSWORD);
