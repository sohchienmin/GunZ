...
bool ZGameInterface::OnCommand(MCommand* pCommand)
{
	...
	case MC_MATCH_RESPONSE_SELECT_CHAR:
	{
		...
		if (nResult == MOK)
		{
			...
			ZGetMyInfo()->SetBPPlus(pCharInfo->nBPPlus);
			ZGetMyInfo()->SetEventCoins(pCharInfo->nEventCoins);
#endif
	...
	case MC_MATCH_RESPONSE_CHARACTER_ITEMLIST:
	{
		...
		pCommand->GetParameter(&nBountyPlus, 1, MPT_INT);
		pCommand->GetParameter(&nEventCoins, 2, MPT_INT);
		ZGetMyInfo()->SetBP(nBounty);
		ZGetMyInfo()->SetBPPlus(nBountyPlus);
		ZGetMyInfo()->SetEventCoins(nEventCoins);
		...
		// gambleitem
		pParam = pCommand->GetParameter(5);
	...
	case MC_MATCH_RESPONSE_BRING_ACCOUNTITEM:
		...
		break;
		case MC_MATCH_RESPONSE_BUY_EVENT:
		{
			int nMessage;
			int nNewBounty;
			pCommand->GetParameter(&nMessage, 0, MPT_INT);
			pCommand->GetParameter(&nNewBounty, 1, MPT_INT);

			if (nNewBounty < 0)
			{
				ZApplication::GetGameInterface()->ShowErrorMessage(nMessage);
			}
			else
			{
				ZApplication::GetGameInterface()->ShowMessage(nMessage);
				ZGetMyInfo()->SetEventCoins(nNewBounty);
			}
		}
		break;
		case MC_MATCH_RESPONSE_BUY_CASH_ITEM:
		{
			int nMessage;
			int nNewBounty;
			pCommand->GetParameter(&nMessage, 0, MPT_INT);
			...
			{
				ZApplication::GetGameInterface()->ShowMessage(nMessage);
				ZGetMyInfo()->SetBPPlus(nNewBounty);
			}
		}
		break;
