...
bool ZPostCommand(MCommand* pCmd) 
{
	...
	else
	{
		...
		if (cmdId == MC_ADMIN_ANNOUNCE ||
			cmdId == MC_ADMIN_ALLCOMMANDS ||
			cmdId == MC_ADMIN_ALLCOMMANDS_NOTARGET ||
			cmdId == MC_ADMIN_CHAT || 
			...
			cmdId == MC_MATCH_REQUEST_BUY_CASH_ITEM ||
			cmdId == MC_MATCH_REQUEST_BUY_EVENT ||
...
void ZGameClient::OnChannelChat(const MUID& uidChannel, char* szName, char* szChat, int nGrade, int nR, int nG, int nB)
{
	...
	if (!ZGetGameClient()->GetRejectNormalChat() || (strcmp(szName, ZGetMyInfo()->GetCharName()) == 0))
	{
...
void ZGameClient::OnChannelList(void* pBlob, int nCount)
{
	...
	for (int i = 0; i<nCount; i++)
	{
		...
		MButton* pButton = (MButton*)pResource->FindWidget("LockMyClanChannel");
		if (pButton)
			pButton->Enable(pNode->bIsOwner);
		pButton = (MButton*)pResource->FindWidget("UnlockMyClanChannel");
		if (pButton)
			pButton->Enable(pNode->bIsOwner);

		MLabel* pLabel = (MLabel*)pResource->FindWidget("PlayerOnlineLabel");
		char szText[128];
...
void ZGameClient::OnStageChat(const MUID& uidChar, const MUID& uidStage, char* szChat, int nR, int nG, int nB)
{
	...
	string name = GetObjName(uidChar);
	//if (find(Ignores.begin(), Ignores.end(), name.c_str()) != Ignores.end()) return;
	MCOLOR ChatColor = MCOLOR(nR, nG, nB);
	...
	char szText[512];
	if (!ZGetGameClient()->GetRejectNormalChat() || (strcmp(pObjCache->GetName(), ZGetMyInfo()->GetCharName()) == 0))
	{
...
void ZGameClient::OnResponseIgnoreList(void* pBlob, int nCount)
{
	...
	for (int i = 0; i<nCount; i++){
		...
		if (pList) {
			...
			pList->AddIgnore(state, pNode->szName, pNode->szDescription);
		}
		else {
			if (ZApplication::GetGameInterface()->GetState() != GUNZ_LOBBY)
			{
...
//			unsigned long nChecksum = MGetMatchItemDescMgr()->GetChecksum();
			...
			ZPostLogin(szID, szPassword, nChecksum, szEncryptMD5Value/*, szEncryptSystemMD5Value*/);
			mlog("Login Posted\n");
		}
...
{
	char szText[256];
	if (find(Ignores.begin(), Ignores.end(), pszPlayerName) != Ignores.end()) return;
