...
public:
	...
	int					GetEventCoins() { return m_nEventCoins; }
	int					GetCash() { return 0; }
	int					GetBPPlus() { return m_nBPPlus; }
	int					GetRed()
	{
		return m_nRed;
	}
	...
	bool				IsAdminGrade() {
		if ((GetUGradeID() == MMUG_EVENTMASTER) || (GetUGradeID() == MMUG_DEVELOPER) || (GetUGradeID() == MMUG_ADMIN) || (GetUGradeID() == MMUG_HEADSTAFF))
			return true;
		else
			return false;
	}
	...
	void SetRed(int nValue)
	{
		m_nRed = nValue;
	}
	void SetGreen(int nValue)
	{
		m_nGreen = nValue;

	}
	void SetBlue(int nValue)
	{
		m_nBlue = nValue;
	}
	void SetEventCoins(int nEventCoins)				{ m_nEventCoins = nEventCoins; }
	void SetBPPlus(int nBP)							{ m_nBPPlus = nBP; }
