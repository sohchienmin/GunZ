...
#define TEXT_COLOR_TITLE			0xFFFFFFFF
...
struct ZScoreBoardItem : public CMemPoolSm<ZScoreBoardItem>{
	...
	int nAP; 
	int fDamage;
	ZScoreBoardItem(const MUID& _uidUID, char* _szLevel, char *_szName, char *_szClan, int _nTeam, bool _bDeath, int _nExp, int _nKills, int _nDeaths, int _nPing, int _nDTLastWeekGrade, bool _bMyChar, bool _bGameRoomUser, bool _bCommander = false, int _nHP = 0, int _nAP = 0, int _fDamage = 0)
	{
		uidUID=_uidUID;
...
bool ZCombatInterface::OnCreate()
{
	...
	if (ZGetMyInfo()->IsAdminGrade()) {
		...
		if (pCache && (pCache->GetUGrade()==MMUG_EVENTMASTER || pCache->GetUGrade()==MMUG_HEADSTAFF)&& pCache->CheckFlag(MTD_PlayerFlags_AdminHide)) {
...
void ZCombatInterface::DrawHPAPNumbers(MDrawContext* pDC)
{
	...
	if (!ZGetConfiguration()->GetExtra()->bInfoBar) return;
	...
	float linespace = 0.071f / 3.f;
	if (ZGetGame()->GetMatch()->IsTeamPlay() && ZGetGame()->GetMatch()->GetRoundState() == MMATCH_ROUNDSTATE_PLAY)
	{
		for (ZCharacterManager::iterator itor = ZGetGame()->m_CharacterManager.begin(); itor != ZGetGame()->m_CharacterManager.end(); ++itor)
		{
			...
			if (pChar->GetTeamID() == ZGetGame()->m_pMyCharacter->GetTeamID() && pChar != ZGetGame()->m_pMyCharacter) {
				...
				char szBuffer[64];
				sprintf(szBuffer, "%s:", (pChar->GetName()).c_str());
				pDC->SetColor(124, 252, 0);
				pDC->SetFont(MFontManager::Get("FONTc8b"));
				TextRelative(pDC, 0, textY, szBuffer);
				textY += 20.f / 600.f;

				if (pChar->GetStatus().Ref().nHP > 0)
				{
...
void ZCombatInterface::OnDraw(MDrawContext* pDC)
{
	...
	if (ZGetGame()->m_pMyCharacter->GetStatus().Ref().ESP == 1)
		bDrawAllPlayerName = true;
			...
			for (ZCharacterManager::iterator itor = ZGetCharacterManager()->begin(); itor != ZGetCharacterManager()->end(); ++itor)
			{
				...
				else if ( pCharacter->GetUID() == pDuel->QInfo.m_uidChallenger)
				{
					if (ZGetMyUID() != pDuel->QInfo.m_uidChallenger)
					{
						if ((pCharacter->GetUserGrade() == MMUG_ADMIN) || (pCharacter->GetUserGrade() == MMUG_DEVELOPER) || (pCharacter->GetUserGrade() == MMUG_EVENTMASTER) || (pCharacter->GetUserGrade() == MMUG_HEADSTAFF))
						sprintf(charName[0], "%s %s", ZMsg(MSG_CHARINFO_LEVELMARKER), pCharacter->GetUserName());
						else sprintf(charName[0], "%s%d  %s", ZMsg(MSG_CHARINFO_LEVELMARKER), pCharacter->GetProperty()->nLevel, pCharacter->GetUserName());
					}
				...
				else if (pCharacter->GetUID() == pDuel->QInfo.m_WaitQueue[0])
				{
					...
					else sprintf(charName[1], "%s %s", ZMsg(MSG_CHARINFO_LEVELMARKER), pCharacter->GetUserName());
				}
				// Waiting 2
				else if (pCharacter->GetUID() == pDuel->QInfo.m_WaitQueue[1])
				{
					...
					else sprintf(charName[2], "%s %s", ZMsg(MSG_CHARINFO_LEVELMARKER), pCharacter->GetUserName());
				}
			}
		}
...
void ZCombatInterface::SetPickTarget(bool bPick, ZCharacter* pCharacter)
{
	...
	if (bPick)
	{
		...
		if (bFriend == false) 
		{
			...
			m_pTargetLabel->SetTextColor(0xffff0000);
			SetPicked(true);
			SetPickedInfo(pCharacter->GetCharInfo()->szName);

			//rvector p1 = pCharacter->GetPosition();
			//rvector p2 =  pCharacter->GetDirection();

			//RDrawLine(p1, p2, 0xFF0000FF);
		}

		m_pTargetLabel->SetTextColor(MCOLOR(pCharacter->GetCharInfo()->nRedColor, pCharacter->GetCharInfo()->nGreenColor, pCharacter->GetCharInfo()->nBlueColor));
	...
	if (ZGetGame()->GetMatch()->IsTeamPlay())
	{
			...
			itor != ZGetGame()->m_CharacterManager.end(); ++itor)
		{
			...
			if (isInViewFrustum(&box, RGetViewFrustum()))
			{
				...
				pDC->SetColor(MCOLOR(pCharacter->GetCharInfo()->nRedColor, pCharacter->GetCharInfo()->nGreenColor, pCharacter->GetCharInfo()->nBlueColor));
...
//	int nMaxLineCount=int((fHeight-fTitleHeight)*(float)RGetScreenHeight()/((float)pFont->GetHeight()*1.1f));
		...
		itor != ZGetCharacterManager()->end(); ++itor)
	{
		...
		else if (pCharacter->IsHeadStaffName())
			sprintf(pItem->szGrade, "%s", "Head Staff");
		else if (pCharacter->IsEventMasterName())
			sprintf(pItem->szGrade, "%s", "GameMaster");
		...
		pItem->nHP = pCharacter->GetStatus().Ref().nHP;
		pItem->nAP = pCharacter->GetStatus().Ref().nAP;
		pItem->fDamage = pCharacter->GetStatus().Ref().nDamage;
...
//		backgroundy=newbackgroundy;
		...
		x = ITEM_XPOS[3] - 0.035f;

		if (pItem->nTeam == ZGetGame()->m_pMyCharacter->GetTeamID() || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_QUEST || ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_SURVIVAL)
			sprintf(szText, "%d/%d   %d", pItem->nHP, pItem->nAP, pItem->fDamage);
		else
			sprintf(szText, "", pItem->nHP, pItem->nAP, pItem->fDamage);
		TextRelative(pDC, x, texty, szText, true);
...
void ZCombatInterface::GameCheckPickCharacter()
{
	...
	if(pMyChar && pMyChar->m_pVMesh) {
		...
		if (!ZGetConfiguration()->GetExtra()->bCharacterVisibility)
		{
			if (bPick) {
