...
void ZPlayerMenu::SetupMenu(ZPLAYERMENU_SET nMenuSet)
{
	...
	if (nMenuSet == ZPLAYERMENU_SET_LOBBY) {
		...
		*/
		...
		AddMenuItem(new ZPlayerMenuItem(ZCMD_PLAYERMENU_IGNORE_ADD, "Ignore"));
	...
	else if (nMenuSet == ZPLAYERMENU_SET_IGNORE) {
		AddMenuItem(new ZPlayerMenuItem(ZCMD_PLAYERMENU_IGNORE_REMOVE, "Remove"));
	}
...
bool ZPlayerMenuListener::OnCommand(MWidget* pWidget, const char* szMessage)
{
	...
	case ZCMD_PLAYERMENU_KICK:
		...
		return true;
	case ZCMD_PLAYERMENU_IGNORE_ADD:
	{
		vector<string>::iterator i = find(ZGetGameClient()->Ignores.begin(), ZGetGameClient()->Ignores.end(), pMenu->GetTargetName());
		if (i == ZGetGameClient()->Ignores.end())
		{
			ZPostIgnoreAdd(pMenu->GetTargetName());
			ZGetGameClient()->Ignores.push_back(pMenu->GetTargetName());
		}
	}
		break;
	case ZCMD_PLAYERMENU_IGNORE_REMOVE:
	{
		ZPostIgnoreRemove(pMenu->GetTargetName());
		ZPostIgnoreList();
	}
		break;
