...
void ZStageInterface::OnCreate( void)
{
	...
	if ( pPicture)
	{
		...
		((MBitmapR2*)m_pItemListFrameImg)->Create( "itemlistframe.tga", RGetDevice(), "interface/default/listframeopen.png");
	...
	{ // 릴레이맵 "RelayMap" 초기화
		...
		{ // f릴레이맵 리스트 배경 이미지
			...
			((MBitmapR2*)m_pRelayMapListFrameImg)->Create( "relaymaplistframe.tga", RGetDevice(), "interface/default/listframeopen.png");
...
//		ZApplication::GetGameInterface()->EnableWidget( "StageMaxPlayer", false);
	...
	BEGIN_WIDGETLIST("Stage_OptionFrame2", pResource, MButton*, pButton);
	pButton->Enable(!bReady);
	END_WIDGETLIST();
	...
	BEGIN_WIDGETLIST("EquipmentCaller2", pResource, MButton*, pButton);
	pButton->Enable(!bReady);
	END_WIDGETLIST();

	BEGIN_WIDGETLIST("EquipmentCaller4", pResource, MButton*, pButton);
	pButton->Enable(!bReady);
	END_WIDGETLIST();
