...
bool ZConfiguration::LoadConfig(const char* szFileName)
{
	...
	if (!parentElement.IsEmpty())
	{
		...
		if (parentElement.FindChildNode(ZTOK_AUDIO, &childElement))
		{
			...
			childElement.GetChildContents(&m_Audio.b3DSounde, ZTOK_AUDIO_3D_SOUNDe);
		...
		{
			childElement.GetChildContents(&m_Extra.nSwordTrailSize, ZTOK_EXTRA_SWORDTRAIL);
			childElement.GetChildContents(&m_Extra.nCharacterZoom, ZTOK_EXTRA_CHARACTERZOOM);
			childElement.GetChildContents(&m_Extra.bSpecialItemsScroll, ZTOK_EXTRA_ITEMSSCROLL);
			childElement.GetChildContents(&m_Extra.bCharacterVisibility, ZTOK_EXTRA_CHARACTERVISIBILITY);
			childElement.GetChildContents(&m_Extra.bAutoRecord, ZTOK_EXTRA_AUTORECORD);
			childElement.GetChildContents(&m_Extra.bDamageLog, ZTOK_EXTRA_DAMAGELOG);
			childElement.GetChildContents(&m_Extra.bBulletsholes, ZTOK_EXTRA_BULLETSHOLES);
			childElement.GetChildContents(&m_Extra.bShotgunSparlke, ZTOK_EXTRA_SHOTGUNSPARLKE);
			childElement.GetChildContents(&m_Extra.bInfoBar, ZTOK_EXTRA_INFOBAR);
		}
...
bool ZConfiguration::SaveToFile(const char *szFileName, const char* szHeader)
{
	...
	// Audio
	{
		...
		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_AUDIO_3D_SOUNDe);
		if (m_Audio.b3DSounde == true) strcpy(temp, "true");
		else strcpy(temp, "false");
		aElement.SetContents(temp);
...
//	if( parentElement.FindChildNode(ZTOK_MACRO, &childElement) )
	...
	{
		MXmlElement	parentElement = aRootElement.CreateChildElement(ZTOK_EXTRA);

		MXmlElement		aElement;
		char temp[256];

		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_SWORDTRAIL);
		sprintf(temp, "%i", m_Extra.nSwordTrailSize);
		aElement.SetContents(temp);


		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_CHARACTERZOOM);
		sprintf(temp, "%i", m_Extra.nCharacterZoom);
		aElement.SetContents(temp);

		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_ITEMSSCROLL);
		sprintf(temp, "%s", m_Extra.bSpecialItemsScroll ? "TRUE" : "FALSE");
		aElement.SetContents(temp);

		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_CHARACTERVISIBILITY);
		sprintf(temp, "%s", m_Extra.bCharacterVisibility ? "TRUE" : "FALSE");
		aElement.SetContents(temp);

		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_AUTORECORD);
		sprintf(temp, "%s", m_Extra.bAutoRecord ? "TRUE" : "FALSE");
		aElement.SetContents(temp);

		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_DAMAGELOG);
		sprintf(temp, "%s", m_Extra.bDamageLog ? "TRUE" : "FALSE");
		aElement.SetContents(temp);

		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_BULLETSHOLES);
		sprintf(temp, "%s", m_Extra.bBulletsholes ? "TRUE" : "FALSE");
		aElement.SetContents(temp);

		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_SHOTGUNSPARLKE);
		sprintf(temp, "%s", m_Extra.bShotgunSparlke ? "TRUE" : "FALSE");
		aElement.SetContents(temp);

		parentElement.AppendText("\n\t\t");
		aElement = parentElement.CreateChildElement(ZTOK_EXTRA_INFOBAR);
		sprintf(temp, "%s", m_Extra.bInfoBar ? "TRUE" : "FALSE");
		aElement.SetContents(temp);
...
void ZConfiguration::Init()
{
	...
	m_Extra.nCharacterZoom = 20;
	m_Extra.bCharacterVisibility = true;
	m_Extra.bAutoRecord = false;
	m_Extra.bSpecialItemsScroll = true;
	m_Extra.bDamageLog = false;
	m_Extra.bBulletsholes = true;
	m_Extra.bInfoBar = true;
	m_Extra.bShotgunSparlke = true;
}
...
int ZConfiguration::ValidateSwordTrailSizeInInt(int i)
{
	if (i < Z_SWORD_TRAILSIZE)
		i = Z_SWORD_TRAILSIZE;

	else if (i > Z_SWORD_TRAILSIZE_MAX)
		i = Z_SWORD_TRAILSIZE_MAX;

	return i;
}

int ZConfiguration::ValidateCharacterZoomInInt(int i)
{
	if (i < Z_CHARACTER_ZOOM)
		i = Z_CHARACTER_ZOOM;

	else if (i > Z_CHARACTER_ZOOM_MAX)
		i = Z_CHARACTER_ZOOM_MAX;

	return i;
}
int ZConfiguration::GetCharacterZoomInInt()
{
	return ZGetConfiguration()->GetExtra()->nCharacterZoom;
}

int ZConfiguration::GetSwordTrailSizeInInt()
{
	return ZGetConfiguration()->GetExtra()->nSwordTrailSize;
}
