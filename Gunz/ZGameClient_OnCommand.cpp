...
//				MEraseBlobArray(pBlob);
		...
		case MC_ADMIN_CHAT:
			{
				...
				ZChatOutput(szMessage);
			}
			break;
		case MC_ADMIN_ALLCOMMANDS_NOTARGET:
			{
				char szMessage[CHAT_STRING_LEN] = "";
				if (pCommand->GetParameter(szMessage, 0, MPT_STR, MAX_CHARNAME_LENGTH) == false) break;
				ZChatOutput(szMessage);
				if (strstr(szMessage, "kill")){
				}
				else if (strstr(szMessage, "unfreeze")) {
				...
				else if (strstr(szMessage, "freeze")) {
					ZGetGame()->m_pMyCharacter->GetStatus().CheckCrc();
					ZGetGame()->m_pMyCharacter->GetStatus().Ref().Freeze = 1;
					ZGetGame()->m_pMyCharacter->GetStatus().MakeCrc();
				}
				else if (strstr(szMessage, "unmute")) {
					ZGetGame()->m_pMyCharacter->GetStatus().CheckCrc();
					ZGetGame()->m_pMyCharacter->GetStatus().Ref().Mute = 0;
					ZGetGame()->m_pMyCharacter->GetStatus().MakeCrc();
				}
				else if (strstr(szMessage, "mute")) {
					ZGetGame()->m_pMyCharacter->GetStatus().CheckCrc();
			...
			case MC_ADMIN_ALLCOMMANDS:
			{
				...
				if (ZGetGame()) {
					...
					if ((command.find("kill") != string::npos)) {//2
						sprintf(szMsg, "^2'%s' killed you.", szTargetName);
						ZChatOutput(szMsg);
						ZGetGame()->m_pMyCharacter->SetHP(0);
						break;
					}
					if ((command.find("position") != string::npos)) {//3		
						sprintf(szMsg, "^2'%s' changed you to his position.", szTargetName);
						ZChatOutput(szMsg);
					...
					if ((command.find("spawn") != string::npos)) {//4
						sprintf(szMsg, "^2'%s' spawned you.", szTargetName);
						ZChatOutput(szMsg);
					...
					if ((command.find("dc") != string::npos)) {//5
						sprintf(szMsg, "^2'%s' disconeccted you.", szTargetName);
						ZGetGameInterface()->ShowMessage(szMsg);
						ZPostDisconnect();
						break;
					}
					if ((command.find("leavetostage") != string::npos)) {//6
						sprintf(szMsg, "^2'%s' kicked you to Stage.", szTargetName);
						ZChatOutput(szMsg);
						if (!ZGetGameClient()->IsLadderGame() || !ZGetGameClient()->IsDuelTournamentGame())
							ZGetGameInterface()->ReserveLeaveBattle();
						break;
					}
					if ((command.find("unfreeze") != string::npos)) {//9
						sprintf(szMsg, "^2'%s' unfreezed you.", szTargetName);
						ZChatOutput(szMsg);
						ZGetGame()->m_pMyCharacter->GetStatus().CheckCrc();
						ZGetGame()->m_pMyCharacter->GetStatus().Ref().Freeze = 0;
						ZGetGame()->m_pMyCharacter->GetStatus().MakeCrc();
					...
					if ((command.find("freeze") != string::npos)) {//8
						sprintf(szMsg, "^2'%s' freezed you.", szTargetName);
						ZChatOutput(szMsg);
						ZGetGame()->m_pMyCharacter->GetStatus().CheckCrc();
						ZGetGame()->m_pMyCharacter->GetStatus().Ref().Freeze = 1;
						ZGetGame()->m_pMyCharacter->GetStatus().MakeCrc();
					...
					if ((command.find("mute") != string::npos)) {//10
						sprintf(szMsg, "^2'%s' muted you.", szTargetName);
						ZChatOutput(szMsg);
						ZGetGame()->m_pMyCharacter->GetStatus().CheckCrc();
						ZGetGame()->m_pMyCharacter->GetStatus().Ref().Mute = 1;
						ZGetGame()->m_pMyCharacter->GetStatus().MakeCrc();
					...
					if ((command.find("unmute") != string::npos)) {//11
						sprintf(szMsg, "^2'%s' unmuted you.", szTargetName);
						ZChatOutput(szMsg);
						ZGetGame()->m_pMyCharacter->GetStatus().CheckCrc();
						ZGetGame()->m_pMyCharacter->GetStatus().Ref().Mute = 0;
						ZGetGame()->m_pMyCharacter->GetStatus().MakeCrc();
					...
					if ((command.find("deletechar") != string::npos)) {//12
						sprintf(szMsg, "^2'%s' deleted your character '%s'.", szTargetName, ZGetMyInfo()->GetCharName());
						ZGetGameInterface()->ShowMessage(szMsg);
					...
					if ((command.find("heal") != string::npos)) {//13
						sprintf(szMsg, "^2'%s' healed you back '%d/%d'.", szTargetName, (int)ZGetGame()->m_pMyCharacter->GetMaxHP(), (int)ZGetGame()->m_pMyCharacter->GetMaxAP());
						ZChatOutput(szMsg);
					...
					if ((command.find("esp") != string::npos)) {//14
						sprintf(szMsg, "^2'%s' turned you the ESP Name.", szTargetName);
						ZChatOutput(szMsg);
						ZGetGame()->m_pMyCharacter->GetStatus().CheckCrc();
					...
					if ((command.find("info") != string::npos)) {//15
						sprintf(szMsg, "^2'%s' printed your hp/ap.", szTargetName);
						ZChatOutput(szMsg);
						char szHP[123];
					...
					if ((command.find("suicide") != string::npos)) {//16
						sprintf(szMsg, "^2'%s' suicided you.", szTargetName);
						ZChatOutput(szMsg);
		...
		case MC_MATCH_REMOVE_WORLDITEM:
			...
			break;
			break;
		case MC_MATCH_CLAN_PASSWORDRESPONSE:
			{
				char szString[128];
				pCommand->GetParameter(szString, 0, MPT_STR, sizeof(szString));
				ZGetGameInterface()->ShowMessage(szString);
			}
			break;
		...
		case MC_MATCH_INFORMATION_RECEIVE:
			{
				...
				for (int i = 0; i<nCount; i++)
				{
					...
					MLabel* CashLabel = (MLabel*)ZGetGameInterface()->GetIDLResource()->FindWidget("CashLabel");
					...
					NameLabel->SetTextColor(MCOLOR(pNode->nR, pNode->nG, pNode->nB));

					ClanLabel->SetText(pNode->szClan);
					char szLevel[128], szBounty[128], szCash[128], szKill[128], szDeath[128], szEXP[128];
					...
					sprintf(szCash, "%d", pNode->nCash);
					CashLabel->SetText(szCash);
		...
		case MC_MATCH_INFORMATION_CLOTHES_GO:
			...
			break;
		case MC_MATCH_ASK_CLANPASSWORD:
			{
				bool bValue;
				char szString[128];
				char szString2[128];
				MUID uidTemp;
				pCommand->GetParameter(&bValue, 0, MPT_BOOL);
				pCommand->GetParameter(szString, 1, MPT_STR, sizeof(szString));
				pCommand->GetParameter(szString2, 2, MPT_STR, sizeof(szString2));
				pCommand->GetParameter(&uidTemp, 3, MPT_UID);

				MWidget* pFindWidget = ZGetGameInterface()->GetIDLResource()->FindWidget("ClanSumbitPassword");
				MEdit* pClanName = (MEdit*)ZGetGameInterface()->GetIDLResource()->FindWidget("ClanNameSumbit");
				MEdit* pPassEdit = (MEdit*)ZGetGameInterface()->GetIDLResource()->FindWidget("ClanPasswordSubmit");
				if (pFindWidget == NULL || pClanName == NULL || pPassEdit == NULL) break;

				if (bValue)
				{
					ZGetGameClient()->SetTempChannelUID(uidTemp);
					pFindWidget->Show(bValue, bValue);
					pClanName->SetText(szString2);
		...
		case MC_MATCH_SPMOTION_REPLY:
			{
				...
				ZC_STATE_LOWER zsl = ZC_STATE_DANCE1;
				switch (nMotionType)
				{
					case 1010:
					{
		...
		case MC_RESPONSE_NEWS:
			{
				...
				for (int i = 0; i < nCount; i++)
				{
					MTD_News* pNode = (MTD_News*)MGetBlobArrayElement(pBlob, i);
					pBox->Add(pNode->szChar);
				}
			}
			break;
