...
bool MBMatchServer::OnCommand(MCommand* pCommand)
{
	...
	case MC_MATCH_REQUEST_BUY_ITEM:
		...
		break;
	case MC_MATCH_REQUEST_BUY_EVENT:
		{
			MUID uidPlayer = pCommand->GetSenderUID();
			unsigned long int nItemID;
			...
			if (NULL == pCharInfo) break;

			MMatchItemDesc* pItemDesc = MGetMatchItemDescMgr()->GetItemDesc(nItemID);
			if (pItemDesc == NULL) break;
			if (pItemDesc->IsCashItem()) break;
			if (!pItemDesc->IsEventItem()) break;
			if (pItemDesc->m_nBountyPrice.Ref() < 0) break;

			int nEventCoins = GetDBMgr()->GetCurrentEventCoins(pObj->GetAccountInfo()->m_nAID);
			if (nEventCoins < 0) break;

			if (nEventCoins - (pItemDesc->m_nBountyPrice.Ref()) < 0)
			{
				MCommand* pCmd = CreateCommand(MC_MATCH_RESPONSE_BUY_EVENT, MUID(0, 0));
				pCmd->AddParameter(new MCmdParamInt(20027));
				pCmd->AddParameter(new MCmdParamInt(-500));
				RouteToListener(pObj, pCmd);
				break;
			}
			else
			{
				if ((GetDBMgr()->UpdateCurrectEventCoins(pObj->GetAccountInfo()->m_nAID, nEventCoins - (pItemDesc->m_nBountyPrice.Ref()))) && (GetDBMgr()->UpdateRentPeriod(0, pObj->GetAccountInfo()->m_nAID, nItemID)))
				{

					MCommand* pCmd = CreateCommand(MC_MATCH_RESPONSE_BUY_EVENT, MUID(0, 0));
					pCmd->AddParameter(new MCmdParamInt(2018));
					pCmd->AddParameter(new MCmdParamInt(nEventCoins - (pItemDesc->m_nBountyPrice.Ref())));
					RouteToListener(pObj, pCmd);
					break;
				}
			}
		...
		break;
	case MC_MATCH_REQUEST_BUY_CASH_ITEM:
		{
			MUID uidPlayer = pCommand->GetSenderUID();
			unsigned long int nItemID;
			int	nItemCount;
			...
			pCommand->GetParameter(&nItemID, 1, MPT_UINT);
			pCommand->GetParameter(&nItemCount, 2, MPT_UINT);
			pCommand->GetParameter(&IsUnlimited, 3, MPT_BOOL);
			...
			if(!pItemDesc->IsCashItem()) break;
			if (pItemDesc->m_nBountyPrice.Ref() < 0) break;
			if (pItemDesc->m_nUnlimitedPrice.Ref() < 0) break;

			int nDonationCoins = GetDBMgr()->GetCurrentCash(pObj->GetAccountInfo()->m_nAID);
			if (nDonationCoins < 0) break;

			if (IsUnlimited)
			{
				if (nDonationCoins - pItemDesc->m_nUnlimitedPrice.Ref() < 0)
				{
					MCommand* pCmd = CreateCommand(MC_MATCH_RESPONSE_BUY_CASH_ITEM, MUID(0, 0));
					pCmd->AddParameter(new MCmdParamInt(20024));
					pCmd->AddParameter(new MCmdParamInt(-500));
					RouteToListener(pObj, pCmd);
					break;
				}
				...
				{
					if ((GetDBMgr()->UpdateCurrentCash(pObj->GetAccountInfo()->m_nAID, nDonationCoins - pItemDesc->m_nUnlimitedPrice.Ref())) && (GetDBMgr()->UpdateRentPeriod(0, pObj->GetAccountInfo()->m_nAID, nItemID)))
					{
						MCommand* pCmd = CreateCommand(MC_MATCH_RESPONSE_BUY_CASH_ITEM, MUID(0, 0));
						pCmd->AddParameter(new MCmdParamInt(2018));
						pCmd->AddParameter(new MCmdParamInt(nDonationCoins - pItemDesc->m_nUnlimitedPrice.Ref()));
						RouteToListener(pObj, pCmd);
						break;
					}
				}
			...
			{
				if (nDonationCoins - (pItemDesc->m_nBountyPrice.Ref()*nItemCount) < 0)
				{
					MCommand* pCmd = CreateCommand(MC_MATCH_RESPONSE_BUY_CASH_ITEM, MUID(0, 0));
					pCmd->AddParameter(new MCmdParamInt(20024));
					pCmd->AddParameter(new MCmdParamInt(-500));
				...
				{
					if ((GetDBMgr()->UpdateCurrentCash(pObj->GetAccountInfo()->m_nAID,nDonationCoins - (pItemDesc->m_nBountyPrice.Ref()*nItemCount) )) && (GetDBMgr()->UpdateRentPeriod(nItemCount * 24, pObj->GetAccountInfo()->m_nAID, nItemID)))
					{

						MCommand* pCmd = CreateCommand(MC_MATCH_RESPONSE_BUY_CASH_ITEM, MUID(0, 0));
						pCmd->AddParameter(new MCmdParamInt(2018));
						pCmd->AddParameter(new MCmdParamInt(nDonationCoins - (pItemDesc->m_nBountyPrice.Ref()*nItemCount)));
						RouteToListener(pObj, pCmd);
						break;
					}
				}
