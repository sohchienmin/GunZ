...
void MBMatchServer::OnRequestBuyItem(const MUID& uidPlayer, const unsigned long int nItemID, const int nItemCount)
{
	...
	if (MGetMatchShop()->IsSellItem(nItemID))
	{
		...
		if (pItemDesc->IsCashItem()) return;
		if (!pItemDesc->IsUnLimitItem()) {
			nRentHourPeriod = pItemDesc->m_nMaxRentPeriod.Ref() * 24;
		}
...
bool MBMatchServer::ResponseCharacterItemList(const MUID& uidPlayer)
{
	...
	pNew->AddParameter(new MCommandParameterInt(GetDBMgr()->GetCurrentCash(pObj->GetAccountInfo()->m_nAID)));
	pNew->AddParameter(new MCommandParameterInt(GetDBMgr()->GetCurrentEventCoins(pObj->GetAccountInfo()->m_nAID)));
	// 장비한 아이템 전송
	int nIndex = 0;
...
void MBMatchServer::ResponseEquipItem(const MUID& uidPlayer, const MUID& uidItem, const MMatchCharItemParts parts)
{
	...
	if ((pObj->GetAccountInfo()->m_nUGrade < 251) && strstr(pItem->GetDesc()->m_pMItemName->Ref().m_szItemName, "Staff"))
	{
	...
	if (strstr(pItem->GetDesc()->m_pMItemName->Ref().m_szItemName, "Staff") && (!strstr(pChannel->GetName(), "Event")))
	{
		MCommand* pNew = CreateCommand(MC_MATCH_RESPONSE_EQUIP_ITEM, uidPlayer);
		pNew->AddParameter(new MCommandParameterInt(MERR_CANNOT_EQUIP_ITEM));
		PostSafeQueue(pNew);
