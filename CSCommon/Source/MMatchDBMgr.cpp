...
TCHAR g_szDB_ADD_IGNORE[] = _T("{CALL spAddIgnore (%d, %d, %d)}");

// 인자: CID, FriendCID
TCHAR g_szDB_REMOVE_IGNORE[] = _T("{CALL spRemoveIgnore (%d, %d)}");

// 인자: CID
TCHAR g_szDB_GET_IGNORE_LIST[] = _T("{CALL spGetIgnoreList (%d)}");
...
TCHAR g_szDB_UPDATE_SERVER_STATUS_NETMARBLE[] = _T("{CALL spUpdateServerStatus (%d, %d)}");
...
TCHAR g_szDB_CREATE_CLAN[] = _T("{CALL spCreateClan ('%s', %d, '%s')}");/*, %d, %d */
...
// Param :	@AID		INT
//			@CharNum	SMALLINT
TCHAR g_szDB_CREATE_CHAR[] = _T("{CALL spInsertChar (%d, %d, '%s', %d, %d, %d, %d, %d, %d, %d)}");
...
bool MMatchDBMgr::CreateCharacter(int* pnOutResult,
	...
	const int nHair, const int nFace, const int nCostume, int nR, int nG, int nB)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	try {
		string strNewName = m_DBFilter.Filtering(string(szNewName));
		strSQL.Format(g_szDB_CREATE_CHAR, nAID, nCharIndex, &strNewName[0], nSex, nHair, nFace, nCostume, nR, nG, nB);
		rs.Open(strSQL, CRecordset::forwardOnly, CRecordset::readOnly);
	}
	catch (CDBException* e) {
		ExceptionHandler(strSQL, e);
...
bool MMatchDBMgr::GetAccountInfo( const unsigned long int nAID, MMatchAccountInfo* poutAccountInfo, const int nServerID )
{
	...
	poutAccountInfo->m_nDonationCoins = rs.Field("DonationCoins").AsInt();
	poutAccountInfo->m_nEventCoins = rs.Field("EventCoins").AsInt();
...
TCHAR g_szDB_SELECT_LATEST_NEWS[] = _T("{CALL spSelectLatestNews(%d)}");
void MMatchDBMgr::GetLatestNews(MUID uidPlayer)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return;
	CString strSQL;
	...
	bool bException = false;
	try {
		strSQL.Format(g_szDB_SELECT_LATEST_NEWS, 0);
		rs.Open(strSQL, CRecordset::forwardOnly, CRecordset::readOnly);
	}
	catch (CDBException* e) {
		bException = true;
	...
	if (rs.IsOpen() == FALSE) { return; }
	char buff[128];
	for (; !rs.IsEOF(); rs.MoveNext()) {
		strcpy(buff, rs.Field("News").AsString());

		MCommand* pNew = MMatchServer::GetInstance()->CreateCommand(MC_RESPONSE_NEWS, uidPlayer);
		void* pNewsFrame = MMakeBlobArray(sizeof(MTD_News), 1);
		MTD_News* pStore = (MTD_News*)MGetBlobArrayElement(pNewsFrame, 0);
		strcpy(pStore->szChar, buff);
		pNew->AddParameter(new MCommandParameterBlob(pNewsFrame, MGetBlobArraySize(pNewsFrame)));
		MEraseBlobArray(pNewsFrame);
		MMatchServer::GetInstance()->Post(pNew);
	}
...
bool MMatchDBMgr::IgnoreAdd(const int nCID, const int nFriendCID, const int nFavorite)
{
	_STATUS_DB_START;
	...
	// 친구 추가
	CString strSQL;
	strSQL.Format(g_szDB_ADD_IGNORE, nCID, nFriendCID, nFavorite);
...
bool MMatchDBMgr::IgnoreRemove(const int nCID, const int nFriendCID)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	try {
		strSQL.Format(g_szDB_REMOVE_IGNORE, nCID, nFriendCID);
		m_DB.ExecuteSQL(strSQL);
	}
	catch (CDBException* e) {
...
bool MMatchDBMgr::IgnoreGetList(const int nCID, MMatchIgnoreInfo* pFriendInfo)
{
	...
	//	return false;
	CString strSQL;
	strSQL.Format(g_szDB_GET_IGNORE_LIST, nCID);
	CODBCRecordset rs(&m_DB);
	...
	for (; !rs.IsEOF(); rs.MoveNext())
	{
		nFriendCID = (int)rs.Field("IgnoreCID").AsInt();
...
bool MMatchDBMgr::CreateClan(const TCHAR* szClanName, const TCHAR* szClanPassword, const int nMasterCID, bool* boutRet, int* noutNewCLID)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	string strClanPassword = m_DBFilter.Filtering(string(szClanPassword));
	strSQL.Format(g_szDB_CREATE_CLAN, &strClanName[0], nMasterCID, &strClanPassword[0]);
...
bool MMatchDBMgr::GetItemTable(map<int, MMatchItemDescForDatabase*>& ItemMapFromDatabase)
{
	...
	for( ; ! rs.IsEOF(); rs.MoveNext() ) 
	{
		...
		//if (rs.Field("IsEventItem").AsInt() == 0)		pItemDesc->m_bIsEventItem = false;
		//else if (rs.Field("IsEventItem").AsInt() == 1)	pItemDesc->m_bIsEventItem = true;
...
TCHAR g_szDB_UPDATE_RENTPERIOD[] = _T("INSERT INTO AccountItem (AID, ItemID, RentDate, RentHourPeriod, Cnt) Values(%d, %d, GETDATE(), %d, %d)");
bool MMatchDBMgr::UpdateRentPeriod(const int nRentTime, const int nAID, const int nItemID)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	try
	{
		strSQL.Format(g_szDB_UPDATE_RENTPERIOD, nAID, nItemID, nRentTime, 1);
		m_DB.ExecuteSQL(strSQL);
	}
	catch (CDBException* e)
	{
...
TCHAR g_szDB_UPDATE_REMOVEPASSWORD[] = _T("UPDATE Clan set Password='' WHERE CLID=%d");
bool MMatchDBMgr::RemovePasswordByName(const int nCLID)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	try
	{
		strSQL.Format(g_szDB_UPDATE_REMOVEPASSWORD, nCLID);
		m_DB.ExecuteSQL(strSQL);
	}
	catch (CDBException* e)
	{
...
TCHAR g_szDB_UPDATE_UPDATEPASSWORD[] = _T("UPDATE Clan set Password='%s' WHERE CLID=%d");
bool MMatchDBMgr::UpdatePasswordByName(char* szText, const int nCLID)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	try
	{
		string strPassword = m_DBFilter.Filtering(string(szText));
		strSQL.Format(g_szDB_UPDATE_UPDATEPASSWORD, &strPassword[0], nCLID);
		m_DB.ExecuteSQL(strSQL);
	}
	catch (CDBException* e)
	{
...
TCHAR g_szDB_GET_CLAN_PASSWORD[] = _T("{CALL spGetClanByPassword ('%s', '%s')}");
bool MMatchDBMgr::GetClanPasswordByName(char* szClanName, char* szClanPassword)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	string strClanName = m_DBFilter.Filtering(string(szClanName));
	string strClanPassword = m_DBFilter.Filtering(string(szClanPassword));
	strSQL.Format(g_szDB_GET_CLAN_PASSWORD, &strClanName[0], &strClanPassword[0]);
	...
	_STATUS_DB_END(0);
	strcpy(szClanName, rs.Field("Name").AsString());
	return true;
}
TCHAR g_szDB_UPDATE_CLANMEMBER_CLANWAR[] = _T("UPDATE ClanMember set ClanWar=%d WHERE CID=%d");
bool MMatchDBMgr::UpdateClanMemberClanWar(const int nCID, const int nStatus)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	try
	{
		strSQL.Format(g_szDB_UPDATE_CLANMEMBER_CLANWAR, nStatus, nCID);
		m_DB.ExecuteSQL(strSQL);
	}
	catch (CDBException* e)
	{
...
TCHAR g_szDB_GET_CLANMEMBER_CLANWAR[] = _T("{CALL spGetClanWarStatus (%d, %d)}");
bool MMatchDBMgr::GetClanMemberClanWar(int nCID, const int nStatus)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;

	CString strSQL;
	strSQL.Format(g_szDB_GET_CLANMEMBER_CLANWAR, nCID, nStatus);
	...
	_STATUS_DB_END(0);
	nCID = rs.Field("CID").AsInt();
	return true;
}
TCHAR g_szDB_UPDATE_CLANMEMBER_CLANCHAT[] = _T("UPDATE ClanMember set Chat=%d WHERE CID=%d");
bool MMatchDBMgr::UpdateClanMemberChat(const int nCID, const int nStatus)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	try
	{
		strSQL.Format(g_szDB_UPDATE_CLANMEMBER_CLANCHAT, nStatus, nCID);
		m_DB.ExecuteSQL(strSQL);
	}
	catch (CDBException* e)
	{
...
TCHAR g_szDB_GET_CLANMEMBER_CLANCHAT[] = _T("{CALL spGetClanChatStatus (%d, %d)}");
bool MMatchDBMgr::GetClanMemberChat(int nCID, const int nStatus)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;

	CString strSQL;
	strSQL.Format(g_szDB_GET_CLANMEMBER_CLANCHAT, nCID, nStatus);
...
TCHAR g_szDB_SELECT_CURRENT_CASH[] = _T("{CALL spGetCurrentCash (%d)}");
int MMatchDBMgr::GetCurrentCash(int nAID)
{
	_STATUS_DB_START;
	...
	bool bException = false;
	try {
		strSQL.Format(g_szDB_SELECT_CURRENT_CASH, nAID);
		rs.Open(strSQL, CRecordset::forwardOnly, CRecordset::readOnly);
	}
	catch (CDBException* e) {
		bException = true;
	...
	if (rs.IsOpen() == FALSE) { return 0; }
	for (; !rs.IsEOF(); rs.MoveNext()) {
		return rs.Field("DonationCoins").AsInt();
	}

	_STATUS_DB_END(10);
	return 0;
}
TCHAR g_szDB_SELECT_CURRENT_EVENTCOINS[] = _T("{CALL spGetCurrentEventCoins (%d)}");
int MMatchDBMgr::GetCurrentEventCoins(int nAID)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return 0;
	CString strSQL;
	...
	bool bException = false;
	try {
		strSQL.Format(g_szDB_SELECT_CURRENT_EVENTCOINS, nAID);
		rs.Open(strSQL, CRecordset::forwardOnly, CRecordset::readOnly);
	}
	catch (CDBException* e) {
		bException = true;
	...
	if (rs.IsOpen() == FALSE) { return 0; }
	for (; !rs.IsEOF(); rs.MoveNext()) {
		return rs.Field("EventCoins").AsInt();
	}
...
TCHAR g_szDB_UPDATE_CURRENT_EVENTCOINS[] = _T("UPDATE Account set EventCoins=%d WHERE AID=%d");
bool MMatchDBMgr::UpdateCurrectEventCoins(const int nAID, const int nCash)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	CString strSQL;
	try
	{
		strSQL.Format(g_szDB_UPDATE_CURRENT_EVENTCOINS, nCash, nAID);
		m_DB.ExecuteSQL(strSQL);
	}
	catch (CDBException* e)
	{
...
TCHAR g_szDB_UPDATE_CURRENT_CASH[] = _T("UPDATE Account set DonationCoins=%d WHERE AID=%d");
bool MMatchDBMgr::UpdateCurrentCash(const int nAID, const int nCash)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	CString strSQL;
	try
	{
		strSQL.Format(g_szDB_UPDATE_CURRENT_CASH, nCash, nAID);
		m_DB.ExecuteSQL(strSQL);
	}
	catch (CDBException* e)
	{
...
//TCHAR g_szDB_GET_NEXTREWARD[] = _T("{CALL spGetNextReward (%d)}");
//bool MMatchDBMgr::GetNextReward(const unsigned long int nAID, MMatchAccountInfo* poutAccountInfo)
//{
//	_STATUS_DB_START;
//	if (!CheckOpen()) return false;
//
//
//	CString strSQL;
//	strSQL.Format(g_szDB_GET_NEXTREWARD, nAID);
//
//	CODBCRecordset rs(&m_DB);
//
//	bool bException = false;
...
//		return false;
//	}
//	poutAccountInfo->m_NextTime.wDay = static_cast<WORD>(rs.Field("NextTimeDay").AsShort());
//	poutAccountInfo->m_NextTime.wHour = static_cast<WORD>(rs.Field("NextTimeHour").AsShort());
//	poutAccountInfo->m_NextTime.wMinute = static_cast<WORD>(rs.Field("NextTimeMin").AsShort());
//
//	_STATUS_DB_END(5);
//
//	return true;
