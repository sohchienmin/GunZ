...
void MMatchServer::OnRequestCreateChar(const MUID& uidPlayer, const int nCharIndex, const char* szCharName,
	const unsigned int nSex, const unsigned int nHair, const unsigned int nFace, const unsigned int nCostume, int nR, int nG, int nB)
{
	MMatchSex sex = (nSex == 0) ? MMS_MALE : MMS_FEMALE;
	ResponseCreateChar(uidPlayer, nCharIndex, szCharName, MMatchSex(nSex), nHair, nFace, nCostume, nR, nG, nB);
}

bool MMatchServer::ResponseCreateChar(const MUID& uidPlayer, const int nCharIndex, const char* szCharName,
	MMatchSex nSex, const unsigned int nHair, const unsigned int nFace, const unsigned int nCostume, int nR, int nG, int nB)
{
	MMatchObject* pObj = GetObject(uidPlayer);
	...
	if (pObj->GetAccountInfo()->m_nUGrade == MMUG_FREE)
	{
		nR = 255;
		nG = 255;
		nB = 255;
	...
	MAsyncDBJob_CreateChar* pJob = new MAsyncDBJob_CreateChar(uidPlayer, pObj->GetAccountInfo()->m_nAID,
		szCharName, nCharIndex, nSex, nHair, nFace, nCostume, nR, nG, nB);
...
void MMatchServer::ResponseMySimpleCharInfo(const MUID& uidPlayer)
{
	...
	pMyCharInfo->nBPPlus = GetDBMgr()->GetCurrentCash(pObj->GetAccountInfo()->m_nAID);
	pMyCharInfo->nEventCoins = GetDBMgr()->GetCurrentEventCoins(pObj->GetAccountInfo()->m_nAID);
...
void MMatchServer::OnIgnoreAdd(const MUID& uidPlayer, const char* pszName)
{
	MMatchObject* pObj = GetObject(uidPlayer);
	if (!IsEnabledObject(pObj)) return;

	if (pObj->GetIgnoreInfo() == NULL) return;
	...
	MMatchIgnoreNode* pNode = pObj->GetIgnoreInfo()->Find(pszName);
	if (pNode) {
		NotifyMessage(uidPlayer, MATCHNOTIFY_IGNORE_ALREADY_EXIST);
		return;
	}
	if (pObj->GetIgnoreInfo()->m_IgnoreList.size() > MAX_IGNORE_COUNT) {
		NotifyMessage(uidPlayer, MATCHNOTIFY_IGNORE_TOO_MANY_ADDED);
		return;
	}
	...
	if (m_MatchDBMgr.IgnoreAdd(nCID, nFriendCID, 0) == false) {
		mlog("DB Query(FriendAdd) Failed\n");
		return;
	}

	pObj->GetIgnoreInfo()->Add(nFriendCID, 0, pszName);
	NotifyMessage(uidPlayer, MATCHNOTIFY_IGNORE_ADD_SUCCEED);
}

void MMatchServer::OnIgnoreRemove(const MUID& uidPlayer, const char* pszName)
{
	MMatchObject* pObj = GetObject(uidPlayer);
	if (!IsEnabledObject(pObj)) return;
	if (pObj->GetIgnoreInfo() == NULL) return;
	...
	MMatchIgnoreNode* pNode = pObj->GetIgnoreInfo()->Find(pszName);
	if (pNode == NULL) {
		NotifyMessage(uidPlayer, MATCHNOTIFY_GENERAL_USER_NOTFOUND);
		return;
	}
	...
	if (m_MatchDBMgr.IgnoreRemove(nCID, nFriendCID) == false) {
		mlog("DB Query(FriendRemove) Failed\n");
		return;
	}

	pObj->GetIgnoreInfo()->Remove(pszName);
	NotifyMessage(uidPlayer, MATCHNOTIFY_IGNORE_REMOVE_SUCCEED);
}

void MMatchServer::OnIgnoreList(const MUID& uidPlayer)
{
	MMatchObject* pObj = GetObject(uidPlayer);
	if (!IsEnabledObject(pObj)) return;

	if (!pObj->DBIgnoreListRequested())
	{
		MAsyncDBJob_IgnoreList* pJob = new MAsyncDBJob_IgnoreList(uidPlayer, pObj->GetCharInfo()->m_nCID);
		pJob->SetIgnoreInfo(new MMatchIgnoreInfo);
		pObj->m_DBJobQ.DBJobQ.push_back(pJob);
	}
	else if (!pObj->GetIgnoreInfo())
	{
		return;
	}

	IgnoreList(uidPlayer);
}

void MMatchServer::IgnoreList(const MUID& uidPlayer)
{
	MMatchObject* pObj = GetObject(uidPlayer);
	if (!IsEnabledObject(pObj)) return;
	if (pObj->GetIgnoreInfo() == NULL) return;
	pObj->GetIgnoreInfo()->UpdateDesc();

	MMatchIgnoreList* pList = &pObj->GetIgnoreInfo()->m_IgnoreList;
	void* pListArray = MMakeBlobArray(sizeof(MIGNORELISTNODE), (int)pList->size());
	int nIndex = 0;
	for (MMatchIgnoreList::iterator i = pList->begin(); i != pList->end(); i++) {
		MMatchIgnoreNode* pNode = (*i);
		MIGNORELISTNODE* pListNode = (MIGNORELISTNODE*)MGetBlobArrayElement(pListArray, nIndex++);
		pListNode->nState = pNode->nState;
		strcpy(pListNode->szName, pNode->szName);
		strcpy(pListNode->szDescription, pNode->szDescription);
	}

	MCommand* pCmd = CreateCommand(MC_MATCH_RESPONSE_IGNORELIST, MUID(0, 0));
	pCmd->AddParameter(new MCommandParameterBlob(pListArray, MGetBlobArraySize(pListArray)));
	MEraseBlobArray(pListArray);
	RouteToListener(pObj, pCmd);
}
