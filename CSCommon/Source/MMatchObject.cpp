...
{ 
	...
	m_bDBIgnoreListRequested = false;
...
void MMatchObject::FreeIgnoreInfo()
{
	if (m_pIgnoreInfo) {
		delete m_pIgnoreInfo;
		m_pIgnoreInfo = NULL;
	}
	m_bDBIgnoreListRequested = false;
}
...
void MMatchObject::SetIgnoreInfo(MMatchIgnoreInfo* pIgnoreInfo)
{
	m_bDBIgnoreListRequested = true;
	m_pIgnoreInfo = pIgnoreInfo;
}
...
void MMatchObject::OnDead(bool bCount)
{
	MMatchServer* pServer = MMatchServer::GetInstance();
