...
void MAsyncDBJob_CreateChar::Run(void* pContext)
{
	...
	if (m_nResult == MOK)
	{
		if (!pDBMgr->CreateCharacter(&m_nResult, m_nAID, m_szCharName, m_nCharNum, m_nSex, m_nHair, m_nFace, m_nCostume, m_nRed, m_nGreen, m_nBlue))
		{
			SetResult(MASYNC_RESULT_FAILED);
		}
	}
...
void MAsyncDBJob_CreateClan::Run(void* pContext)
{
	...
	if (!pDBMgr->CreateClan(m_szClanName, m_szClanPassword, m_nMasterCID, &m_bDBResult, &m_nNewCLID))
	{
		SetResult(MASYNC_RESULT_FAILED);
...
bool MAsyncDBJob_CreateClan::Input(const TCHAR* szClanName, const TCHAR* szClanPassword, const int nMasterCID, const MUID& uidMaster)
{
	strcpy(m_szClanName, szClanName);
	strcpy(m_szClanPassword, szClanPassword);
	m_nMasterCID = nMasterCID;
