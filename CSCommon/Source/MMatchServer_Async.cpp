...
void MMatchServer::ProcessAsyncJob()
{
	while(MAsyncJob* pJob = m_AsyncProxy.GetJobResult()) 
	{
		...
		case MASYNCJOB_FRIENDLIST:
			...
			break;			
		case MASYNCJOB_IGNORELIST:
			{
				OnAsyncGetIgnoreList(pJob);
			}
			break;
...
void MMatchServer::OnAsyncGetIgnoreList(MAsyncJob* pJobInput)
{
	MAsyncDBJob_IgnoreList* pJob = (MAsyncDBJob_IgnoreList*)pJobInput;
	...
	pObj->SetIgnoreInfo(pJob->GetIgnoreInfo());	// Save Async Result

	IgnoreList(pObj->GetUID());
}
