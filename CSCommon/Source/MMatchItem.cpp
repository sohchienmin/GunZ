...
m_nColor(0xFFFFFFFF), m_nImageID(0), m_nBulletImageID(0), m_nMagazineImageID(0), m_bIsCashItem(false), m_bIsEventItem(false), m_bIsSpendableItem(false)
{
	...
	m_nUnlimitedPrice.Set_MakeCrc(0);
...
void MMatchItemDesc::ShiftFugitiveValues()
{
	...
	m_nUnlimitedPrice.ShiftHeapPos_CheckCrc();
...
bool MMatchItemDescMgr::ParseItem(MXmlElement& element)
{
	...
	for (int i = 0; i < nAttrCount; i++)
	{
		...
		else if (!stricmp(szAttrName, "unlimited_price"))
		{
			pNewDesc->m_nUnlimitedPrice.Set(atoi(szAttrValue));
			pNewDesc->m_nUnlimitedPrice.MakeCrc();
		}
		else if (!stricmp(szAttrName, "iseventitem"))
		{
			if (!stricmp(szAttrValue, MICTOK_TRUE)) 	pNewDesc->m_bIsEventItem = true;
			else										pNewDesc->m_bIsEventItem = false;
		}
