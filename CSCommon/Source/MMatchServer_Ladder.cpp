...
///////////////////////////////////////////////////////////////////////////
// LadderStage
bool MMatchServer::LadderJoin(const MUID& uidPlayer, const MUID& uidStage, MMatchTeam nTeam, bool duel)
{
	...
	if (!duel) {
		if (ClanRejoiner.count(pObj->GetCharInfo()->m_nCID)) {
...
void MMatchServer::LadderGameLaunch(MLadderGroup* pGroupA, MLadderGroup* pGroupB, int MapID)
{
	...
	for (list<MUID>::iterator i=pGroupA->GetPlayerListBegin(); i!= pGroupA->GetPlayerListEnd(); i++)
	{
		...
		if (pObj) 
		{
			...
			pObj->LastVoteID = -1;
			LadderJoin(uidPlayer, uidStage, MMT_RED, false);
		}
	}
	// B 그룹 입장
	for (list<MUID>::iterator i=pGroupB->GetPlayerListBegin(); i!= pGroupB->GetPlayerListEnd(); i++)
	{
		...
		if (pObj) 
		{
			pObj->PlayerWarsIdentifier = -1;
			pObj->LastVoteID = -1;
			LadderJoin(uidPlayer, uidStage, MMT_BLUE, false);
		}
	}
...
void MMatchServer::DuelLaunch(MLadderGroup* pGroupA)
{
	...
	for (list<MUID>::iterator i=pGroupA->GetPlayerListBegin(); i!= pGroupA->GetPlayerListEnd(); i++)
	{
		...
		if (pObj) 
		{
			...
			if (count > size/2) 
			{
				LadderJoin(uidPlayer, uidStage, MMT_BLUE, true);
			}
			else {
				LadderJoin(uidPlayer, uidStage, MMT_RED, true);
			}
		}
		count++;
	}
...
void MMatchServer::OnLadderRequestChallenge(const MUID& uidRequestMember, void* pMemberNamesBlob, unsigned long int nOptions, unsigned long int nAntiLead, unsigned long int nDodge, bool warmUp)
{
	if ((MGetServerConfig()->GetServerMode() != MSM_LADDER) && 
		(MGetServerConfig()->GetServerMode() != MSM_CLAN)) return;
	...
	if (nDodge == 1)
	{
		pGroup->SetDodgeMatching(true);
	}
	else
	{
		pGroup->SetDodgeMatching(false);
	}
