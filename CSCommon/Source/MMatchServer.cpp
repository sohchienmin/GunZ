...
bool MMatchServer::CompareMatchItem(MMatchItemDescForDatabase *pItem1, MMatchItemDesc *pItem2)
{
	...
	if (pItem1->m_bIsEventItem != pItem2->m_bIsEventItem) {
		if (pItem1->m_bIsEventItem) mlog("%d(%s) - IsEventItem does not match(DB : true)\n", pItem1->m_nID, pItem2->m_pMItemName->Ref().m_szItemName);
		else						mlog("%d(%s) - IsEventItem does not match(DB : false)\n", pItem1->m_nID, pItem2->m_pMItemName->Ref().m_szItemName);
		bResult = false;
	}
