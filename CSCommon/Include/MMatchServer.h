...
protected:
	...
	void OnAsyncGetIgnoreList(MAsyncJob* pJobInput);
...
protected:
	...
	bool ChannelJoin(const MUID& uidPlayer, const MUID& uidChannel, const char* pszChannelPassword = NULL);
	...
	void OnRequestChannelJoin(const MUID& uidPlayer, const MUID& uidChannel, const char* pszChannelPassword);
...
protected:
	...
	void OnRequestCreateChar(const MUID& uidPlayer, const int nCharIndex, const char* szCharName,
		...
		const unsigned int nCostume, int nR, int nG, int nB);
...
protected: // 친구
	...
	void OnIgnoreAdd(const MUID& uidPlayer, const char* pszName);
	void OnIgnoreRemove(const MUID& uidPlayer, const char* pszName);
	void OnIgnoreList(const MUID& uidPlayer);
	void IgnoreList(const MUID& uidPlayer);
protected:	// 클랜
	...
	void OnClanRequestCreateClan(const MUID& uidPlayer, const char* szClanName, const char* szClanPassword);
