...
public:
	...
	bool CreateCharacter(int* pnOutResult,						// 결과값
		...
		const int nHair,						// 헤어
		const int nFace,						// 얼굴
		const int nCostume, int nR, int nG, int nB);					// 초기 코스튬
	...
	int GetCurrentEventCoins(int nAID);
	bool UpdateCurrectEventCoins(const int nAID, const int nCash);
	//bool GetNextReward(const unsigned long int nAID, MMatchAccountInfo* poutAccountInfo);
	// 아이템 관련	
	...
	bool IgnoreAdd(const int nCID, const int nFriendCID, const int nFavorite);
	bool IgnoreRemove(const int nCID, const int nFriendCID);
	bool IgnoreGetList(const int nCID, MMatchIgnoreInfo* pFriendInfo);
	...
	bool CreateClan(const TCHAR* szClanName, const TCHAR* szClanPassword, const int nMasterCID, bool* boutRet, int* noutNewCLID);
	...
	bool UpdateRentPeriod(const int nRentTime, const int nAID, const int nItemID);
	bool RemovePasswordByName(const int nCLID);
	bool UpdatePasswordByName(char* szText, const int nCLID);
	bool UpdateClanMemberClanWar(const int nCID, const int nStatus);
	bool GetClanMemberClanWar(int nCID, const int nStatus);
	bool UpdateClanMemberChat(const int nCID, const int nStatus);
	bool GetClanMemberChat(int nCID, const int nStatus);
	bool GetClanPasswordByName(char* szClanName, char* szClanPassword);
#ifdef _DEBUG
	bool test();
#endif	
};
