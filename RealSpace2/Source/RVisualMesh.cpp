...
void RVisualMesh::RenderWeapon()
{
	if( m_WeaponVisualMesh[ m_SelectWeaponMotionType ] ) {
		...
		if(btrack_render) {
			DrawTracks(m_bDrawTracks, pVWMesh, 0, m, m_nTracksTrailSize);
		...
		if(b2hCheck) {
			...
			if(btrack_render) {
				DrawTracks(m_bDrawTracks, pVWMesh, 1, m, m_nTracksTrailSize);
				DrawEnchant(pVWMesh,0,m);
			}
		}
	}
...
void RVisualMesh::DrawTracks(bool draw, RVisualMesh* pVWMesh, int mode, rmatrix& m, int nSize)
{
...
void RVisualMesh::SetDrawTracks(bool s, int n) {
	m_bDrawTracks = s;
	if (n != m_nTracksTrailSize)//the code you gave me.
	{
		if (m_pTracks[0] != NULL)
		{
			delete m_pTracks[0];
